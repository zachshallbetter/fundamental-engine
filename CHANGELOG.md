...
- **The lazy overlay surface no longer builds a canvas it can never paint, and sizes at one DPR ceiling instead of two** (Field Surfaces program §6.1, C-24 + C-25 — groundwork for #672/#673).
  - **C-24**: `ensureOverlaySurface()` asked the `overlayCanvasProvider` for a canvas *before* checking whether the field had a drawing context. A signals-only field (`render: 'none'`, the default since #538) can never paint a reading — the frame gate requires a context — so on `<field-root>` that built a full-viewport `mix-blend-mode` layer and added it to the compositing tree to show nothing. The context check now comes first. This is a deferral, not a removal: `setRender()` acquires the context and calls straight back, so a field that later leaves `'none'` still gets the reading it asked for.
  - **C-25**: the same path sized the backing store from the **raw** host DPR, ignoring `dprCap` and the adaptive quality tier's ceiling — both of which the underlay has always applied. Measured on the unfixed engine: an 800px viewport at host DPR 3 with `dprCap: 2` produced a **2400px** overlay backing store against the underlay's 1600, on the most expensive surface in the tree, and it stayed that way until the next resize. Both ceilings now come from one `effectiveDpr()` used by both surfaces, so they cannot disagree about what a CSS pixel is worth.
  - **The C-25 path is narrower than the program's write-up implies**, which two vacuous test drafts established before the real one: an *injected* `overlayBackend` is bound at construction and never enters the lazy path at all, and an *eager* `overlayCanvas` on a field leaving `render: 'none'` is re-sized by `sizeSurfaces` within the same call, so it self-corrects. The live case is a field that already draws, given a lazy provider, when a reading is switched on.
