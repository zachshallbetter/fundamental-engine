...
- **NavGrid — obstacle-aware navigation and goal flow-fields** (#439, part 1: the subsystem). Every force in this engine is *reactive*: it reads the local field and pushes. That is right for matter and wrong for an agent with somewhere to be — a reactive seeker walks into the near side of a wall and stays there, because from where it stands the goal is *that way*. FieldKit called it "the rabbit stuck on the wrong side of the fence" and worked around it with barrier-avoidance plus stuck-repick. **No local rule fixes it**: the information needed (there is a way round, and it starts by going the wrong way) is not local.
  ...
  - Ten tests, headed by the literal fence-with-a-gap walked step by step, which fails if the route ever enters geometry or stops short. Four mutations each turn it red: an eight-connected sweep, the diagonal guard removed, obstacles that stop blocking, and a goal inside geometry accepted.
