...
- **`field.pulse(body, energy)` — the transient per-body flare, on its own channel** (#567). The engine had no way to say *"something just happened, here."* Consumers built it anyway, in the wrong place: a hand-rolled 60fps rAF loop that maxed a value into `--d` and decayed it, which meant the app was writing the channel the engine owns, fighting the engine's own eased density over one custom property. `pulse()` is a decaying one-shot on a body, published as **`--field-pulse`** — a separate channel, stepped by the field loop, so a host flares from an ordinary event handler and writes no animation driver at all.
  - It is deliberately neither of the two things it gets confused with, and the tests prove both: **not `burst`** (a blast at screen coordinates that shoves and heats matter — `pulse` leaves every particle bit-identical, checked against a real burst so the comparison is not blind), and **not `--d`** (which *measures* gathered matter — two otherwise identical fields, one flared every frame, produce the same density series exactly).
  - Energy is additive and saturates at 1; the half-life is 0.15 s. The decay runs on **wall time, not the motion budget**: a reduced-motion field is frozen, and its flares still resolve to rest rather than latching on as a highlight nothing can clear.
  - Withheld from the read-only agent view (`forAgent`): it moves no matter, but it drives a channel the page's CSS reacts to, so an agent holding it could flash a body's UI. Registered as engine-owned in the platform lint, so an author binding that *sets* `--field-pulse` is flagged as a second writer — the collision it was split out of.
  - Mirrored on the platform feedback sink (`<field-root>`), `@fundamental-engine/vanilla` and `@fundamental-engine/three`, with a test asserting the platform and engine sinks write the same variables. JS-only for now: the parity matrix records the Swift/Kotlin gap rather than hiding it.
- **`completion-flare` Pattern** (#567) — in `EXPERIMENTAL_PATTERNS`, outside the frozen 64. Pairs the standing half (a gentle engaged `attract` with `data-feedback`, so the flare lands on a body that already has presence) with the imperative flare. Its notes address `burst` by name, because the compiler cannot: "flare the thing" is exactly the intent a reader satisfies with the wrong call.
