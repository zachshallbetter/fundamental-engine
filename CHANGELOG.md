...
- **A seeded field now runs the same way twice** (#1207). `env.t` was `(now - t0) / 1000` with `t0 = performance.now()` captured when the field was **constructed**, so the simulation clock's origin depended on how long elapsed between building a field and its first frame — real wall time, leaking into the frame path. A seeded run was therefore not reproducible. Six identical runs in one process drifted **monotonically**, and three separate processes disagreed in the fifth decimal. The clock is now frame-relative: the first frame is `t = 0` for every caller. `opts.now` still anchors input-idle tracking, which legitimately wants wall time — it simply is no longer secretly required for a repeatable run.
  - **Why it hid.** Seeding the rng removed most of the variance (about a thousandfold), leaving a residue small enough to read as floating-point noise. It is not noise: the drift is monotonic in both coordinates with shrinking increments, which is a quantity converging, not scattering. And the cross-plane golden pins ONE apply of ONE force — the right thing for force math, and structurally unable to see a whole-run property.
  - **Found** while building the mass visual for #1142, whose acceptance needs two dynamic bodies' recoil to differ by their mass ratio *within tolerance* — unwritable against a field that does not reproduce. The surfaced path was `reaction: true`, which is experimental and off by default, so **nothing shipped was affected**; the defect was in the shared frame path regardless.
  - **Behaviour note.** `env.t` now starts at exactly 0 rather than at a small, arbitrary offset, so the time-reading forces (`resonate`'s `sin(t·ω)`, `wind`'s curl noise) begin at a defined phase instead of wherever construction happened to land. The conformance golden is untouched: it supplies `env.t` directly rather than through `frame()`.
  - The regression test is the one the golden cannot be: run the same seeded field three times in one process and compare **bit patterns** (`0 === -0` would pass a naive equality while differing).
...
- **A dedicated mass field on `/demo` — first-class inertia, made visible** (#1142). `/demo` shares Base's page `<field-root>`, so it could not turn on `mass`/`reaction` without imposing them on the whole page. The panel owns its own `bounds:` field, and two plates differ **only in rendered area** — same force, same strength, same range — so any difference in how far they drift is inertia and nothing else. The recoil figures are read with `query()` each tick, not animated. Under reduced motion the engine freezes itself (`dt = 0`) and the caption says so, rather than leaving two zeros that look like a broken read-out.
  - **The obvious experiment does not measure mass**, and the test says so out loud. Two bodies of different size side by side sit in *different matter*, so their recoil differs for two reasons at once: measured that way the ratio comes out near **6.4** against a predicted **4**, and the excess is neighbourhood, not inertia. Holding position constant and varying only area — one body per run, same point, same seeded field — the relationship is exact: a body of 4× the area recoils **0.501×** (predicted 0.5) and one of a quarter the area **1.993×** (predicted 2.0).
  - Four tests: the `1/inertia` scaling, the `clamp(…, 0.4, 4)` at both ends, a control proving `mass: false` makes area irrelevant to nine decimal places, and a guard preserving the side-by-side case as a *qualitative* claim so the confounded version is not re-derived later.
