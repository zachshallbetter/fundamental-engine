...
- **Tab order as a field current — `--field-next` and a directional lean** (#943, deferred from #665). #942 gave keyboard users field *engagement* at mouse parity; this is the path **between** bodies. A keyboard user travels a sequence the page never shows them, where a mouse user can see where they are going before they go. Focusing a body now marks the one Tab reaches next.
  - **`--field-next` is the primary output, and that is a design decision.** This engine is signals-first — `render: 'none'` is the default (#538) — so a purely drawn current would be invisible on most fields. The channel is set to `1` on the successor and **removed, not zeroed**, when it stops being one, so a plain `[style*="--field-next"]` selector works. It is also, by being a value rather than a movement, already the static equivalent the reduced-motion contract requires.
  - The sequence follows the **real tab-order rule**: positive `tabindex` first in ascending order, ties keeping document order, then everything natural in document order, with `tabindex="-1"` excluded entirely. It **never wraps** — the last body's successor is the browser chrome, and pretending the sequence loops would point the cue somewhere Tab will not go.
  - A `[data-hot]` card is usually a plain container whose `<a>` takes the focus, so a body's place in the sequence comes from its **focusable descendant**, not its own `tabIndex`. Reading the container's `-1` dropped every such card out of the order — most of them — which is a bug a realistic test harness caught and an idealised one hid.
  - On a drawn, moving field the matter also **leans along** the focused → next direction, falling off across the channel and tapering at both ends so it reads as a route between two places rather than an infinite line through them. Gated on `env.dt`, so a reduced-motion field contributes nothing and the cue falls back to the channel.
