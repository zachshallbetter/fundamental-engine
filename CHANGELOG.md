...
- **NavGrid — obstacle-aware navigation and goal flow-fields** (#439). Every force in this engine is *reactive*: it reads the local field and pushes. That is right for matter and wrong for an agent with somewhere to be — a reactive seeker walks into the near side of a wall and stays there, because from where it stands the goal is *that way*. FieldKit called it "the rabbit stuck on the wrong side of the fence" and worked around it with barrier-avoidance plus stuck-repick. **No local rule fixes it**: the information needed (there is a way round, and it starts by going the wrong way) is not local.
  - So navigation is a separate subsystem, not another force. An **occupancy grid** marks cells that cannot be entered — impassable, not merely repulsive; a **breadth-first sweep from the goal** fills every reachable cell with its distance; each cell's direction is that field's downhill step. The sweep runs once per goal and serves every agent, which is what makes it a flow *field* rather than a path — a thousand seekers cost one sweep and none needs to know the route.
  - **Four-connected sweep, guarded diagonals.** A diagonal step between two blocked orthogonals squeezes through a seam nothing with a body could pass. The sweep avoids it, and so does the direction — the latter found by a test: a walker clipped a fence at (253, 380) before the direction carried the same guard the sweep already did.
  - Unreachable is *reported*, not papered over: a sealed region answers `navReachable() === false` and a zero direction, so an agent holds still instead of jittering — and a goal placed inside geometry yields no field at all rather than a field pointing at somewhere nothing can reach.
  - **`field.navigate({ goal, obstacles })` and `AgentSpec.navigate`** put it on the field: an agent that opts in steers toward the goal, and the ticket's acceptance holds on a live field — it routes around the fence and arrives, where the same agent without `navigate` presses into the near side and never gets past. Steering is **added** to the forces rather than replacing them, so a navigating agent is still shoved out of a crowd and still finds its way round.
  - **Impassable means impassable.** Steering alone is not enough: a force can shove a navigating agent into a wall, and once inside, the flow field reads zero and it is stuck there — the exact bug this subsystem exists to end. A step that would finish inside geometry is undone. Tested by hurling an agent at the fence at forty times the steering's strength; it stops at the near face.
  - Withheld from the read-only agent view: `navigate` places geometry and a goal every other agent follows, so an agent holding it could wall the field's agents in or walk them somewhere.
  - Ten tests on the grid, headed by the literal fence-with-a-gap walked step by step, which fails if the route ever enters geometry or stops short. Four mutations each turn it red: an eight-connected sweep, the diagonal guard removed, obstacles that stop blocking, and a goal inside geometry accepted.
