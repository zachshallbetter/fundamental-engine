...
- **The Kotlin force catalog is generated, not hand-copied** (#1186). `gen-force-catalog.mjs` emitted the JSON catalog and the Swift one; Kotlin's was maintained by hand, and had drifted: `relief` was **absent entirely** after the force shipped on three planes, and `fieldflow` had become `"Field Flow"` against the passport's `Fieldflow`. Neither was visible to any gate. `android/lab/.../GeneratedForceCatalog.kt` is now emitted from `passport.ts` alongside the other two, and `Catalog.kt` joins it to its hand-written blurbs by token — the blurbs are independent editorial prose (none matches the engine's `meta.desc`), so they are preserved rather than generated. `CatalogCoverageTest` fails on a force with no blurb or a blurb with no force. New `check:force-catalog` gate.
- **The force-catalog parser no longer drops rows in silence.** It required `token`/`label`/`family`/`klass`/`truthMode` in that exact order with nothing between them, so any reordered or newly-inserted field detached a row — and the only guard was `catalog.length < 30`, meaning **seven of 37 forces could vanish on a green run** (measured). Rows are now split brace-balanced and read field-by-name in any order; a row missing a required field is a named error, and the count is checked against the rows actually present rather than a magic floor.


- **Swift: drawing a diagnostic no longer detonates a real body** (#1162, the Swift half of #1155). `forceAt` — the probe behind the streamlines / force-vector overlays, the field-line tracer, `FieldEngine.forceSampler`, and `FieldHandle.sample()` — ran every body's real `apply()` against the **caller's live `Env`**. `sink` is not a pure read (`b.accreted += 1`; `if b.accreted >= b.capacity { e.supernova(b) }`), so on Apple platforms **drawing the field moved it**: a sink's accretion budget advanced and the live `supernova` fired, detonating a body that never absorbed a particle. `diffuse`/`memory` wore the real scalar grids, `wall` threw sparks into the live field, and every sample drew from the simulation's seeded `rng`, so a recorded run drifted and the same point read a different vector depending on what had been drawn before it. **Swift's exposure was unbounded, and worse than the JS engine's**: JS shares one probe particle whose `cap` was never cleared, so `sink`'s own `if p.cap != nil` guard short-circuited for the life of the process and a body was over-accreted exactly once — self-limiting by accident. Swift builds a *fresh* `Particle` per call, so that guard never fired and every probe point inside `absorbR` accreted on every sample of every frame; measured on a 12-frame streamlines walk over one capacity-4 sink, **420 accretions and 417 supernova firings** where the correct count is zero. The probe now runs under its own inert `Env` (`makeProbeEnv(mirroring:)`) — no-op `spark`/`supernova`/`spawn`, scalar grids that read through but drop their deposits, its own per-sample `seededRng` — plus an internal `Env.isProbe` marker that `sink` checks before accreting, because a direct body write goes through no service an inert env could stub. **The drawn vectors are unchanged** for every non-mutating force (`sink` contributes no velocity either way) and the cross-plane conformance golden is byte-identical.

### Changed

- **`read:snapshots` and `read:diagnostics` are now enforced on the agent view — a behaviour change that TIGHTENS access** (#1160). Both were declared in `AgentCapability` on all three planes and consulted by nothing: `forAgent`'s `has()` was called for seven of the nine capabilities, and these two appeared nowhere outside their own type declaration. A caller who deliberately withheld them still received full snapshots — a silent failure in the permissive direction, which is the worst direction for a capability system. Verified against `main` before the fix: an agent view holding **zero** capabilities could call `snapshot({ includeParticles: true })` and receive the field's entire live particle pool. They now gate:
  - **`read:snapshots` gates the CAPTURE SURFACE.** Without it the agent view has **no `snapshot()` at all** — JS omits the member (`AgentFieldView.snapshot` is now optional, matching how `replay`/`focusState` already reflect their grant in the facade's shape); Swift and Kotlin return `nil`/`null`, the idiomatic mirror where a protocol/class member cannot vanish (the shape Kotlin's `influenceAt` already uses). The call is **closed, not emptied**: an empty capture is indistinguishable from an empty field, which is the same silently-permissive reading this gate exists to stop. `query()` is unaffected, so a denied agent is barred from the portable, serializable, replayable artifact — never blinded.
  - **`read:diagnostics` gates the raw particle pool** (`includeParticles`) inside a capture — the engine's own internal state rather than a modelled reading of bodies, relationships or metrics, and the lane the repo's own taxonomy already classifies as diagnostic (`contracts/passport.ts`: *"reveals internal state"*). Withheld, `includeParticles` is forced off **even under `profile: 'debug'`**, which previously re-opened it. The Swift and Kotlin snapshots carry no particle lane yet, so on those planes the gate is wired at the option boundary and becomes observable when that lane lands.

  **Who is affected.** Any caller that withheld either capability and relied on the access it got anyway. **Migration:** add `'read:snapshots'` to the grant of any agent view that calls `snapshot()`, and `'read:diagnostics'` to any that reads `snapshot().particles`. A grant that already listed them behaves exactly as before. TypeScript callers of `view.snapshot(...)` now need `view.snapshot?.(...)` (or a `read:snapshots` grant plus `!`), because the member is optional — the type now states what the runtime enforces. Both gates compose tighten-only with the existing per-lane caps and with `FieldPolicy`; a closed `budgets.agentRead` budget still pins a *granted* capture to the `public` profile. All three planes move together, and each now carries a test asserting the **deny** direction — the absence of exactly that test is how this survived. The cross-plane conformance golden is untouched: no force, integrator or metric behaviour changed, only the grant path. The `/docs/api/imperative` capability table, which shipped these two rows as `DECLARED`, now reads `WIRED`.
### Changed

- **Homepage art direction — the calm-instrument pass** (#914, decisions D1–D5). Site-only; no package or engine change. The homepage's resting liveliness now comes from the DECLARED primitive: `<field-root resting-motion="flow">` replaces the interim site-level ambient-swirl body this layout carried since #1051 (the divergence-free curl from #1110; it draws nothing, is measured as `--temperature`, and scales with `dt`, so reduced motion contributes exactly nothing). The chrome and the field are now art-directed as separate layers (**D1**): inside the field and its readings hue still means *which force*, while the page's chrome and type take two **state** accents — `--state-signal` (signal/query/readout) and `--state-attention` (attention/energy/state change) — which keeps both layers inside canon §5's "hue = categorical meaning" (**D2**). The install cards become instrument cards (**D4**): the frosted-glass pane, drop shadows and hover lift are gone, the accretion glow becomes a flat fill meter, and figures are tabular — the border stays the live `--field-density` consumer, so the cards still read the field without painting one. Type modes are separated (**D5**): `body` no longer resolves its face through `var(--display)` (which is why display and body had collapsed into one visible face), the vestigial `--body` token is retired along with its single usage, and the homepage's readout rules now name the real `--mono` token instead of the undefined `--font-mono` they had been falling through. Also fixes a reduced-motion leak in the shared layout: the global `:focus-visible` ring was an infinite `focus-glow` pulse that was never gated on `prefers-reduced-motion`; it is now a static ring.

### Changed

- **Rust: the conformance golden is now crate-local, so a published crate can prove its own cross-plane parity** (#1045, epic #1036). `scripts/gen-conformance-golden.mjs` writes the Swift fixture and a new `rust/crates/fundamental-core/tests/fixtures/conformance-golden.json` **from one payload**, and the Rust test reads its own copy instead of an `include_str!` that climbed out of the crate root into `swift/`.
  - **Why it mattered.** That climbing path resolves only inside this monorepo, so the conformance test had to be **excluded from the published tarball** — a crates.io consumer got a crate that could not run the one suite proving it agrees with the other planes. It ships now: `cargo package --list` carries both `tests/golden_conformance.rs` and `tests/fixtures/conformance-golden.json`. The `exclude` is gone and the Cargo.toml comment that named this exact follow-up is resolved.
  - **Still a single source of truth.** Both copies are written from the same string in one pass, so a regeneration cannot update one and leave the other behind — which would quietly leave two planes conforming to *different* goldens, exactly the failure the cross-plane gate exists to catch. A new `check:golden:sync` compares the two byte-for-byte, and `check:golden` runs it first.
  - **The committed fixtures were copied, never regenerated.** The crate-local file is a byte-identical copy of the existing Swift one, and both are byte-identical to `origin/main`.

### Added

- **Rust: the CMS reading layer — scores, clusters, relations** (#1044, epic #1036). The plane's product surface: turn a settled field into the three things a CMS actually asks of it. Every reading is attributed back to the caller's own index, because a score that cannot name its record is not usable.
  - **Scores** rank by `Body::count`, the gathered density the integrator measures during the force pass. Normalization puts the leader at 1 and an all-zero field at 0 rather than `NaN` — a field where nothing gathered has no leader, and inventing one by dividing by zero would rank noise. Ties keep the caller's order, so a rebuild of unchanged content does not reshuffle.
  - **Clusters** are single-link connected components over particle proximity, found with union-find (path halving, union by size) over a uniform grid. This reads the field's OWN verdict about what belongs together rather than re-deriving groups from the inputs that produced it — which is the whole reason to run a field instead of clustering the records directly. Membership is sorted and ordering is size-descending with a stable tiebreak, so a reordered pool still clusters identically.
  - **Relations** are symmetric by construction: an asymmetric "A relates to B but not back" is an artifact of top-k truncation, not a fact about the field. Strength is `1/(1 + d/scale)`, bounded in `(0,1]`, so a caller can rank or threshold without knowing the field's units.
  - **One way to misread this, documented at the call site.** `count` is a PER-FRAME accumulator the integrator zeroes at the top of every step. A score therefore describes the frame just run, not the history of the run — read it after `solve` returns and before stepping again.
  - **A worked example** (`cargo run --example cms`) takes five content records through bind → `solve` → read, with a snapshot receipt that replays the run and reproduces the ranking. It settles in 2571 steps, and it demonstrates the point in its own output: "Changelog v0.9" is declared *less* important than "Deep-dive" (0.45 against 0.80) and still outranks it, because it sits between the other two records and inherits the matter they gather. A flat weighting cannot express that. The example uses `attract` rather than `gravity` deliberately — true inverse-square orbits forever, and a rebuild that never reaches equilibrium has no settled state to read.
  - Twelve tests. The conformance golden and the `ranking` example are unchanged. Still experimental and unpublished; no npm, Swift or Kotlin change.

- **Rust: the source pass, mortal matter, and the `spawn` / `morph` forces** (#1038, epic #1036). Adds `Particle.age` (`None` = immortal), `Effect::Spawn`, the integrator's aging and despawn, and the two source/scatter forces. The standard registry moves **34 → 36**, leaving exactly one catalogued force unported: `relief`, whose host-declared scalar channel this plane has no concept for.
  - **`spawn` is the one force that deliberately breaks conservation**, so the budget that bounds it travels with the matter: every emitted particle is mortal. `age: None` is the conserved base field, and the distinction is a **type rather than a sentinel** — there is no lifespan value meaning "lives forever". The emission rate is clamped to `source_cap / life` per frame, so the live population settles near `source_cap` however hard the body pushes: a fountain has a basin, not an ever-rising flood. Fractional rates accumulate on the body, so 0.1 particles per frame still flows instead of rounding to silence.
  - **The `Force::source` hook now takes `&mut Body`.** A source owns per-body emission state — `spawn` carries its fractional rate across frames — and routing that through an effect would have made the body's own bookkeeping a message to the integrator about itself. An earlier draft did exactly that, inventing an effect variant to carry a number back to the body it came from.
  - **Ordering is load-bearing, twice.** Aging runs *after* the source pass, or every emission arrives a frame short of its lifespan. Emissions are added *through the store* rather than pushed at the pool, so each gets a real id — id 0 is invisible to `collide`'s pair gate and to every id-keyed attribution downstream. Despawn uses `retain` rather than swap-remove, so a seeded run's pool order stays reproducible.
  - **`morph` hashes a fixed per-particle scatter fraction to a target index**, so a given particle always aims at the same point. Derived from pool position, the assignment would rehash as the pool reorders and the assembled mark would boil.
  - Eleven tests. One was wrong first: it set `engaged = false` to stop the source and watched the pool keep growing — `spawn` emits while **visible**, not while engaged, which JS says in as many words, so the force was right and the test was asserting folklore. The deferred-forces list is a real assertion again (`["relief"]`; 36 registered + 1 deferred is the whole 37-force catalog, so an empty list would mean the catalog grew unnoticed). The conformance golden and the `ranking` example are unchanged. Still experimental and unpublished; no npm, Swift or Kotlin change.

- **Rust: the field structure hooks, and `fieldflow` to follow them** (#1041, epic #1036). Adds `Force::field(body, x, y)` — the renderable, followable structure a body radiates — implemented for `gravity` (an inward radial well), `charge` (a monopole) and `magnetism` (a dipole), plus `net_field` superposition and the `fieldflow` force that transports matter along the result.
  - **A structure field is geometry, not a force law.** `apply()` is untouched by any of it, and a field line is not a particle path: a sideways-moving particle orbits a gravity well rather than falling down its line. The value of the hook is that one function now serves both consumers — the integrator resolves it per particle for `fieldflow`, and a field-line diagram traces the same call — so the drawn picture *is* the engine's field rather than a hand-rolled stand-in.
  - **Why a monopole and a dipole are different shapes, tested as such.** A lone electric charge radiates, so `charge` is a monopole; magnetic monopoles do not exist, so a magnet loops and `magnetism` is a dipole. The dipole test pins exactly that difference: beyond the `+` pole the field runs outward, but on the perpendicular bisector it returns along the axis — the N→S return path a monopole has no analogue for.
  - **`fieldflow` does two distinct things and the distinction matters**: the STEER turns velocity onto the local tangent without spending it (speed-preserving, like `align`), while the STREAM accelerates along it and therefore does work. Measured: neutral matter streamed from x = 100 to **136.19px** over 30 frames, and a particle launched *perpendicular* to the lines at velocity (0, 20) was turned onto the radial line, reading **(6.91, 0.90)** after 12 frames. The opt-in `charge_gated` flag (#711) reproduces the magnetized-plasma reading: the same run leaves neutral matter at exactly **100.000000** while charged matter still reaches 136.1875. Unlike `magnetism`, ungated `fieldflow` carries neutral matter — that is the point of it.
  - **`Env.field_here` is a resolved value, not an `at(x, y)` accessor, and deliberately so.** The superposition needs every body *and* the force registry, but inside the particle loop the integrator already holds `&mut Body` and `&mut Env`, so a force cannot ask the env to evaluate its siblings — the same constraint that puts the `screen` pass where it is. A tracer that needs the field elsewhere calls the public `net_field` directly. `None` when nothing radiates, which lets a follower distinguish "no field here" from "a field that is zero here": a true null point between two poles is a real, followable feature, and there is a test for the cancellation.
  - The superposition is resolved **only when a `fieldflow` body is present**, so radiating costs nothing on its own. Thirteen tests; the conformance golden and the `ranking` example are unchanged. The standard registry moves **28 → 29** forces. One deliberate omission against JS: JS scales a radiated field by `(1 + Q_GAIN·b.d)` so a body radiates harder as it gathers density — this plane's `Body` has no eased density `d` and its own `apply` omits the same gain, so the field hook omits it too rather than invent a value. Still experimental and unpublished; no npm, Swift or Kotlin change.
- **Rust: the scalar-grid subsystem, and the three class-[C] forces that ride it** (#1039, epic #1036). Ports `ScalarGrid` (`sample` / `deposit` / `gradient` over a uniform grid, with diffuse, wave, slow-decay and held stepping) plus `diffuse`, `propagate` and `memory`. Class [C] means the state lives in a field buffer rather than in the particle or the body: matter influences matter only through what it has left behind, and that indirection *is* the force — it is what makes a trail a trail rather than an attraction.
  - **The grid NAME picks the scheme**, exactly as in the JS field: `wave…` is a leapfrog wave, `memory…` slow decay, `potential:…` a held raster (#443), anything else diffuses. `propagate` asks for `"wave-propagate"` and gets wave stepping purely because of that prefix, so the convention is load-bearing rather than cosmetic and has its own test.
  - **A body-level `Force::source` hook**, run once per frame per body after the particle loop, so a source acts once per frame rather than once per existing particle. `propagate` deposits its shock pulse through it. The ticket scoped this hook to the spawn/morph issue (#1038), but only the *matter-emitting* half belongs there — the hook itself is what `propagate` needs, so it lands here and #1038 builds `Effect::Spawn` and mortal matter on top of it.
  - **`Env.frame_n` was declared from the start and advanced by nothing**, because until now nothing read it. `propagate`'s shock train fires on `frame_n % 12`, so left as it was the pulse would have fired on *every* frame — `0 % n == 0` forever. The integrator now advances it, which is inert for every existing force (none read it) and verified so.
  - **Measured behaviour, not sign checks.** A `propagate` front sweeps matter monotonically outward — radius 100 → 117.6 → 148.9 → 186.5 → 224.7 px over 40 frames, with the grid peak visibly re-pulsing every 12 — and never inward, which is the acceptance criterion (matter rides the gradient *magnitude*, steep only where a front is passing, so no standing bump builds at the source to pull it back). A worn `memory` path pulls **4.5×** harder than a fresh one: 93.0px against 20.8px of displacement over 12 frames.
  - Sixteen tests covering the schemes, the name→mode convention, the Neumann boundary, `decay`/`clear`, held-grid immutability, and non-finite clamping in `fill_from` — a `NaN` cell would reach a velocity through `sample`, and a NaN velocity slips the `speed > c` guard entirely, since every comparison against NaN is false, so the clamp belongs where the untrusted value enters the engine. The subsystem is opt-in: a field naming no class-[C] force allocates no buffer, and the conformance golden and the `ranking` example are both unchanged. The standard registry moves **28 → 31** forces. One deliberate divergence: the buffers are `f64`, not JS's `Float32Array`, because the whole Rust plane is f64 — the schemes, constants and clamps match, the last bits will not. Still experimental and unpublished; no npm, Swift or Kotlin change.
- **Rust: `screen` — the cross-body quiet zone** (#1040, epic #1036). A visible body carrying `screen` damps the magnitude of **other** bodies' forces on matter inside its `range`, by `clamp(1 − S·(1 − d/r)², min, 1)` — text shielded from a noisy field, a calm pocket in a busy one. It is the only modifier that reaches across bodies: `spotlight` and `resonate` bend their own body's siblings and so live in the per-body `modify` hook, while `screen` must see a body it is not attached to. Its `apply` and `modify` are both no-ops and the whole force lives in the integrator's force pass, the only place per-particle, per-body forces compose.
  - **The screen bodies' geometry is lifted out once per frame, before the particle loop takes its mutable borrow of `bodies`.** That is not an optimisation, it is the constraint: a per-body hook holding `&mut Body` cannot look at its siblings, which is precisely why a cross-body modifier cannot be expressed as one. Factors are then computed once per particle and reused across every body's pass, and a screen never damps itself.
  - **Fields that use no screens are untouched** — the collection is empty, the pass is skipped, and this was verified rather than asserted: the cross-plane conformance golden is unchanged and the `ranking` example's output is byte-identical to `origin/main`. Measured attenuation on the conformance scenario's geometry: matter at a screen's core displaced **0.0000px** against **42.6445px** for the identical unshielded pair, and with `screen_min = 0.5` the floored case sits between at **20.0155px**.
  - New `Body.screen_min` (default 0) and `math::screen_factor`, whose values are pinned against the JS reference in a test — nine cases including the rim, the floor, `strength > 1`, and the fact that `range = 0` means a screen shields **nothing** rather than everything (it is not "global" the way a range-0 attractor is). Seven tests; the self-damping guard has its own, and it fails if the index check is removed. The standard registry moves **28 → 29** forces. Still experimental and unpublished; no npm, Swift or Kotlin change.
- **`absorbR` through `BodySpec` — a programmatic body can finally declare its own capture horizon** (#1177). `data-absorb` has always set a body's `absorbR` from markup, but `BodySpec` carried no such field on **any** plane, so a body created through `addBody()` was pinned to whichever default its plane happened to hold. That radius is not cosmetic: capture is gated on `dist < absorbR` (`forces/index.ts`), it is the horizon `sink` absorbs within, the throat `warp` transports through, and the rim `supernova` ejects past. A programmatic sink therefore could not express what one line of markup could. Now `BodySpec.absorbR` (JS), `BodySpec.absorbR: Float?` (Swift) and `BodySpec.absorbR: Float?` (Kotlin) all forward to the backing body; the parity matrix records the surface on all three planes (`absorb-r`, counts 11/9/8 → 12/10/9).
  - **Strictly additive — no default moves.** Omitted, every plane behaves exactly as before: JS routes through the scanner's documented `data-absorb` default of **64**, and the Swift and Kotlin `Body` keep their own default of **10**. Each plane carries a test pinning its own unchanged default, and the JS test is seeded (`rng: seededRng(1)`) because accretion counts are small and an unseeded field varies run to run.
  - **That 6.4× disagreement between the planes is real, pre-existing, and deliberately NOT resolved here** — it is a behaviour change on whichever plane moves, and the conformance harness cannot see it because it sets `absorbR` explicitly on every case. Filed as a maintainer decision rather than silently picked.
- **The fractional `budgets.agentRead` gate is now enforced, not just declared** (#915). `agentRead` shipped with only its `0` boundary wired — a closed budget collapsed the agent surface to ids + shape, and the `0 < b < 1` gradient was carried on the policy and consulted by nothing. A caller who asked for a tenth of the field received all of it. The gradient now means what the policy always implied: **`b` is the share of the field's readable body population one agent view may consume.**
  - **A partial read admits a subset of the bodies, and edges follow their endpoints.** A relationship or influence naming a withheld body is itself a disclosure of that body's id and existence, so an edge survives only when *every* body it names does. `snapshot()` admits exactly the bodies `query()` admits — otherwise the capture surface hands back precisely the field the reading just withheld.
  - **Selection is deterministic, stable per body id, and not positional.** Each property is load-bearing. Deterministic, because a random draw makes an agent read unreplayable, and record/replay is a contract the engine keeps everywhere else. **Stable, because a subset resampled per call leaks the whole field to a patient reader — the union of enough independent 10% samples is 100%. A budget you can defeat by calling `query()` in a loop is not a budget; this is the security core of the gate.** Not positional, because "the first ⌈n·b⌉" leaks scan order and hands every agent the same prefix.
  - **The digest is bit-identical on all three planes**, so the same policy over the same field admits the same bodies wherever it runs. JS, Swift and Kotlin each pin the same ten-vector fixture in their tests. It is FNV-1a over the id's UTF-16 code units followed by a murmur3 `fmix32` avalanche; the finalizer is not cosmetic, because real body ids are near-identical short strings (`body-0`, `body-1`, …) that raw FNV-1a barely mixes in the high bits — which is the entire signal once the digest is read as `h / 2^32`. Measured without it over 64 ids: `b=0.1` admitted **0** bodies, `b=0.25` and `b=0.5` both admitted **24**, and `b=0.75` admitted **all 64**.
  - **Two boundaries are documented rather than silently chosen.** Field-wide `metrics` are aggregates over the whole field, not per-body readings, and stay gated by `read:metrics` alone. The raw particle pool carries no body identity, so there is no stable key to sample it by, and it stays gated by `read:diagnostics` alone.
  - **A port asymmetry, stated plainly.** On Swift and Kotlin the `EdgeRecord` lane (`readEdges()` / `relationships()`) names its endpoints by the body's opaque `data`, not by identity, so a partial read has no key to filter it on. Tighten-only therefore **closes** that lane under a fractional budget; the endpoint-filtered graph is still reachable through `snapshot()`, whose relationship readings are keyed by id. Giving `EdgeRecord` real ids would lift the restriction and is worth a follow-up.
  - **The existing boundaries are unmoved**: unset and `b >= 1` read the whole field, `b <= 0` closes the surface exactly as before. No force, integrator or metric behaviour changed, so the cross-plane conformance golden is untouched. `substrate-api.md` and `agent-safety-model.md` move `agentRead` out of the declared-not-yet-enforced list.
- **Rust: `collide` — elastic pairwise collision, and the impulse effect it needed** (#1037, epic #1036). The first of the deferred Rust forces, and the one that required a new mechanism rather than a transcription. `collide` is the only class-[B] force that must move its **neighbour** as well as its own particle. The JS engine simply mutates it, because there `e.neighbors()` hands back live particle references; the Rust plane reads a frame-start *snapshot*, so writing to it would be a silent no-op. The neighbour's half of the exchange now travels as `Effect::Impulse { particle_id, dv }` — a value the integrator applies by id — which is the same effects-as-data substrate the Field Receipts thesis wants, since an impulse log is exactly an attribution record.
  - **Each pair resolves exactly once, by the lower id.** JS gets this for free a different way: it writes both halves immediately, so by the time the neighbour takes its turn the pair is already separating and the `relN >= 0` guard skips it. That trick needs live neighbour state. Reading a snapshot, *both* particles would see an approaching pair and the impulse would land twice — a head-on elastic pair would separate at twice its closing speed. Gating on `p.id < n.id` resolves it once and makes the outcome **independent of pool order**, which is a stronger guarantee than the JS engine offers: JS's result depends on visit order when three or more bodies touch in one frame.
  - **The integrator's particle loop is now two passes.** Impulses must land on velocity *before* it is integrated into position, or every collision is delayed a frame. Forces and body bookkeeping run in the first pass, impulses drain, then integration runs in the second. This is numerically inert for every other force — neighbours come from the frame-start snapshot, body centres are fixed for the step, and `b.count` is written but never read during the pass — and that was verified rather than assumed: the cross-plane conformance golden is unchanged, and the `ranking` example's output is byte-identical to `origin/main`.
  - Equal-mass, matching JS: the `0.5` in the impulse is the reduced mass of two unit spheres, and the neighbour snapshot carries no mass to do better with. Eight tests cover momentum conservation across restitutions, velocity exchange, the fully-inelastic case, separating and out-of-contact pairs, 3D contacts, order independence, and the resolve-once property; four of them fail if the id gate is removed. The standard registry moves **28 → 29** forces. Still experimental and unpublished; no npm, Swift or Kotlin change.
- **Rust: `solve()` — step to equilibrium, and say honestly whether it got there** (#1042, epic #1036). `step()` is the live-tick primitive, the thing a render host calls sixty times a second. `solve()` is the headless complement and the Rust plane's actual entry point: a CMS rebuild or a batch analysis does not want frames, it wants the settled answer and the same answer every time. It steps until the largest per-particle `|Δvelocity|` over one step falls to `epsilon`, or until `max_iters` runs out, and returns `{ iterations, settled }`.
  - **`settled` is a verdict, not a courtesy.** `false` means the field was still moving when the cap ran out, and whatever you read from it is a snapshot of motion rather than equilibrium. A hand-rolled `for _ in 0..600 { step(…) }` — which is what the `ranking` example does — cannot tell you which of those two you got.
  - **The convergence signal reaches zero at a *moving* fixed point too**, which is the one that matters here: a particle under a constant force settles at terminal velocity, where the force input equals the friction loss, and its `Δv` vanishes there. Measured on a three-particle attract well: 382 iterations at `epsilon = 1e-3`, 653 at `1e-6` (the default), 921 at `1e-9`.
  - **A frozen field never settles.** With `env.dt == 0` the integrator returns immediately (§18), so no velocity ever changes and a naive convergence check would call that instant equilibrium. It is the opposite — the field never ran — so `solve` returns `{ iterations: 0, settled: false }` rather than report a fixed point it never reached. An empty pool, by contrast, settles at the first step: no matter *is* the fixed point.
  - Seven tests, including the three the ticket asks for (an attract field converges; a `thermal`-stirred field runs to the cap and reports `settled: false`; identical inputs give an identical iteration count) plus a guard that `solve` is exactly `step` in a loop — same positions and velocities as stepping manually the number of times it reports. If those diverge, a receipt's "this took N steps" stops describing a reproducible trajectory. Still experimental and unpublished; no npm, Swift or Kotlin change.
- **Rust: deterministic snapshot + causal replay — the Field Receipts substrate** (#1043, epic #1036). `FieldSnapshot::capture` takes everything a step reads as an owned value — the particle pool and its id counter, the bodies, the RNG's exact position, the frame counter and the environment constants — and `replay(snapshot, n)` reproduces exactly what running those `n` steps on the original would have produced. `replay_recording` additionally returns the per-step effect log.
  - **Why this is the receipts substrate.** A receipt has to answer "why did this happen", and the only honest answer is one someone else can re-run. Two properties make that possible here: the RNG is seeded and `Copy`, so a capture takes its exact *position in the stream* rather than a fresh seed; and effects are plain values, so a step's consequences can be recorded alongside the state that produced them. The state says what the field became; the log says what happened to make it so.
  - **A capture stores the live generator, not a seed** — the subtle requirement, and it has a test that only fails for a capture taken *mid-run*. Storing a seed passes trivially at frame 0 and diverges from then on, so the test deliberately burns 30 frames of a `thermal` body's draws before capturing. Verified by breaking `capture` to store a fresh stream: exactly that one test fails, and the frame-0 test keeps passing, which is the point.
  - **The id counter is part of the state.** A restore that rebuilt the pool but not `FieldStore::next_id` would hand re-added matter different ids than the original run, and every id-keyed attribution — an impulse, a receipt line, an effect entry — would then name the wrong particle.
  - **What a snapshot deliberately omits is as considered as what it holds.** `Env::vector` and `Env::dist` are per-particle scratch overwritten before every force call, `effects` is cleared at the top of each step, `capture_request` is consumed within one, and the neighbour snapshot is rebuilt from the pool each frame. None survives a step boundary, so none is state — capturing them would imply a fidelity that is not real.
  - Nine tests, comparing raw IEEE **bit patterns** rather than `==`, because that is what "byte-identical" means: `f64` equality would quietly accept `0.0 == -0.0`. They also pin that a capture is an owned value the live field cannot mutate, that replay is a pure function of its capture, that the effect log reproduces, and that recording does not perturb the outcome. No serialisation format is imposed — the crate has zero dependencies, so a snapshot is a plain owned struct with public fields that a host serialises however it already does. No force, integrator or registry change: the conformance golden and the `ranking` example are untouched. Still experimental and unpublished; no npm, Swift or Kotlin change.
- **Rust: the `fundamental-platform` crate — the host / binding layer** (#1046, epic #1036). The second crate in the Rust workspace: the environment seam, the six-phase frame scheduler (`discover → read → compute → state → write → render`), and data-record binding. `fundamental-core` is its **only** dependency, and the direction is one-way — `core ← platform`, never up.
  - **Headless-first, which is the whole point.** The JS plane's host binds DOM elements because a web page is what it has. A server, a CMS or a batch analysis has *data records* — rows, documents, content items — so the default `DataHost` binds those. Nothing in the crate assumes a screen exists. This is what makes the Rust plane's "a body is a data record rather than a widget" claim concrete rather than aspirational: bind records, run frames, read rankings back **attributed to the row they came from** — a score is worth nothing if you cannot say which document it belongs to.
  - **The layering invariant has its own test** (`dep_direction.rs`), checked against the manifests rather than the code, because that is where a violation would actually land: a `use` of a crate you do not depend on does not compile, so the dependency *edge* is the thing to forbid. It also pins the stronger claim the core makes — **zero runtime dependencies**, not merely "no platform".
  - **Phase order is a hard invariant, and violations surface rather than vanish.** Reading world geometry is legal only in `discover` and `read`; a read during `write` sees a half-updated world — some handlers have written, others have not — so it is a real bug that returns a plausible number. `FrameContext::note_geometry_read` reports it with the operation's name (which read was illegal is the useful half), and strict mode turns it into a hard error. Four tests cover it, including that a legal read is silent (a check that cries wolf gets ignored) and that violations do not leak between frames.
  - **Two defaults deliberately differ from the core's.** A record binds with `feedback: true` where `Body::default()` is `false` — a DOM body is opt-in because measuring costs a layout read the page may not want, but a data record exists *in order to be read*. And on each frame's re-discover the platform carries `accreted` forward: a sink that has swallowed matter for two hundred frames must not be handed back an empty one just because the host re-described its geometry. `count` is not carried, because the integrator zeroes it every frame by design.
  - Sixteen tests including an executable doc example. Zero warnings, zero clippy findings. The core crate is untouched — the conformance golden and the `ranking` example are unchanged. Both crates stay **experimental and unpublished**; the crates.io workflow still publishes `fundamental-core` only. No npm, Swift or Kotlin change.

- **`relief` — height-aware fields: terrain as a cause, on every plane** (#443, FieldKit gap #6). `addField(name, sampler)` shipped the *input* half in 0.5.1 — a host could hand the engine a terrain sampler and read it back — but nothing could read it as a **cause**: `Env` carried no channel accessor, so no force could reach a height channel at all. This lands the coupling half as one new **default-OFF** extended force. A body declaring `relief` admits the named channel as a scalar **potential** Φ = G·h and transports matter down its gradient (`v += −∇Φ·S·(1 − d/r)`), so water drains into the basins of the host's own elevation map and matter climbing a ridge slows — emerging from the field rather than faked by the host each frame. `data-potential` names the channel (default `'height'`), `data-spin < 0` climbs instead of drains, and `data-range="0"` is global, which is usually what terrain wants. New surface: the token `relief`, `data-potential` → `Body.potential` / `BodySpec.potential`, `Env.potential?(name)`, and a `GridMode` `'held'` with the `potential:` grid-name prefix. Mirrored on Swift (`ReliefForce`, `Env.potential`, `Body.potential`, `GridMode.held`) and Kotlin (`ReliefForce`, `Env.potential`, `Body.potential`, `GridMode.HELD`) in the same commit. **The force count moves 36 → 37.** Rust (experimental) has no channel concept and is unchanged.
  - **Default-off, so nothing moves.** `Env.potential` is assigned only while a body actually declares `relief` — the `fieldAt?` / `accum?` precedent — so a field whose host called `addField` but declared no such body is byte-identical to one that did not: **registration still does not couple**, exactly as the docs promise. The cross-plane conformance golden is untouched (`check:golden` is a zero diff; the six pinned forces are unaffected and a host-sampler force cannot join them).
  - **Phase 1 only.** Height biases *planar* flow. Bodies still live at z = 0 and `relief` joins the deliberately-planar list in `z-axis.md`; height as a genuine z coordinate is phase 2, is **not** additive, and #443 is not closed by this.

### Changed

- **The `addField` "never cached" promise is amended, not quietly contradicted** (#443). `system-contracts.md` §2.1, the `FieldHandle.addField` JSDoc and the docs-api row all promised a sampler that is "pull-based — called on demand, **never cached**". `sampleField` still caches nothing, and with no `relief` body declared nothing is cached at all — but a declared potential *is* rasterised into a held grid, so the promise is now narrowed on the record on all three planes. The raster's invalidation set is the whole contract: `addField`, `set()`, `remove()` (which also **drops** the raster, so a withdrawn channel can never be read stale) and resize. Nothing re-rasters on a frame cadence, so held state is never frame-phase dependent — the raster is pulled by the force pass and refilled when the epoch it last saw has moved. Under reduced motion the integrator returns before the force pass, so the raster freezes with the motion with no `dt` special-case.
- **One documented non-byte-identical case:** a host already opening a grid named `potential:…` got a *diffusing* grid before and gets a *held* one now. No shipped code, pattern or doc opens such a name.
- **Kotlin: `modeForName` matched `"memory"` exactly where JS and Swift use a prefix** — a real cross-plane drift, fixed while #443 rewrote that function. No shipped name is affected (the `memory` force opens exactly `"memory"`).
...
- **`createOverlaySurface(root?, opts?)` in `@fundamental-engine/dom`, and `<field-root overlay-blend overlay-z>`** (#721). The front overlay canvas — fixed, full-viewport, click-through, `aria-hidden`, marked `data-field-overlay` — used to be an inline literal in the element; the helper now creates and places it in one call and returns `{ canvas, resize, destroy }`, with `blend` (CSS `mix-blend-mode`, default `screen`), `zIndex` (default `5`), `parent` (default `<body>`), `marker`, and an opt-in `autoSize` (DPR-aware, `dprCap`-capped, follows `window` resize) for a standalone surface — when the canvas is bound to `createField` as `overlayCanvas` / `overlayCanvasProvider`, core still owns the backing-store size, so `autoSize` stays off. `<field-root>` now creates its surface through the helper and gains `overlay-blend` / `overlay-z` attributes (plus `setOverlayBlend` / `setOverlayZ` setters that reflect, as #541), applied to the existing surface in place — never a rebuild. Both values are validated before they reach the surface's inline style (`normalizeOverlayBlend` / `normalizeOverlayZ`, also exported): a blend must be a single CSS keyword and a z-index an integer or `auto`; anything else falls back to the default, so an attribute value can never smuggle a further declaration onto the click-through canvas. The React `<FieldField>` / `useFieldField` overlay canvas is created through the same helper (its two duplicated inline literals are gone), so it now also carries the `data-field-overlay` marker. **Default behaviour is otherwise unchanged**: with no attributes the surface is byte-identical to the previous `z-index:5; mix-blend-mode:screen` inline style, pinned by test.
- **Scoped participation + portals for shadow-DOM hosts** (`@fundamental-engine/core`, `@fundamental-engine/elements`; shadow-dom.md §17–§19, #681). `RegisterBodyDetail` gains two opt-in keys: `scope?: 'nearest' | 'global'` and `field?: 'root' | string | Element`. With `scope: 'nearest'` an event-registered host is adopted only by the field whose scan root is its nearest enclosing `data-field-boundary` (a contained `bounds:` / `containerHost` field), or by the page field when none encloses it — resolved across shadow roots, so a host nested in another component's shadow tree still finds the light-DOM boundary. With `field` the host portals to exactly one field: `'root'` is the page field, a selector or element names a contained field by its scan root; `field` beats `scope`. `ShadowRegistry.bodies(build, root?)` takes the asking field's scan root (`field.ts` now passes `host.root`), and the platform runtime's `registerShadowBody(sink, detail, root?)` applies the same rule to measurement, so simulation and measurement never disagree about ownership. New pure exports: `fieldAdopts`, `nearestFieldBoundary`, `isPageRoot`, and the `FieldScope` / `FieldTarget` types. **Default unchanged**: a registration without either key is built by every field that hears it, exactly as before — pinned byte-for-byte (`shadow.scope.test.ts`: the event path drives a seeded field identically to the light-DOM scan of the same element), and omitting `root` keeps the old signature for headless hosts. This closes the #980 two-writer `--d` flicker on the *event* path only when a host opts in: the light-DOM scanner already defaults to nearest-enclosing ownership, so the event path's `'global'` default is now the odd one out — flipping it to `'nearest'` is a separate maintainer decision. `data-scope` / `data-field` attributes are **not** read by the engine (the site uses `data-field="off"` as a styling hook); they are component-side vocabulary a custom element forwards on its detail. The cross-document-continuity half of #681 is tracked as #679.
- **The `wormhole` preset — two linked throats that actually transport matter** (#660). The last composite the cosmology spec named and never shipped. `<a data-preset="wormhole" data-pair="#b">` expands into three co-located virtual bodies — `attract` (0.9 / 300) draws matter into the throat, `warp` (throat 40) relocates it to the paired mouth, and a `repel` (6 / 200) **gated `when: 'hot'`** throws the arrival clear. **No new force token**: the catalog stays at 36 forces and 20 render modes, and the cross-plane conformance golden is untouched. Presets go **8 → 9**.
  - **Why the gate.** The canon composition (§20.7) is asymmetric — `attract + warp(throat A) ⟷ warp + repel(throat B)` — but a preset expands identically on every element carrying it. The asymmetry is recovered from the matter instead of from a second token: `warp` stamps `heat = 0.6` on everything it relocates and the `hot` gate is `heat > 0.3`, so the repel acts *only* on just-transported matter and is invisible to the cool matter falling in. One token, both roles. A symmetric ungated repel cannot work at all: the same term that ejects also blocks entry (measured: 0 throat crossings).
  - **It transports, and that is measured, not asserted.** Eight particles seeded on a 250px ring around mouth A, driven through the real conformance runner for 1200 frames: **8/8 transported, exactly one throat crossing each, settling 274.7px clear of the exit throat** with particle count conserved (8 → 8) and the result invariant under `data-twist`. For contrast, the same pair with the repel removed ping-pongs — **489 crossings** — because `warp` carries velocity through unrotated, so inbound matter emerges still moving inward. The drafted `attract + warp + lens` composition parked at ≈52px, on the throat's lip.
  - **The `warp` atom is unchanged.** The known exit-side question (emerge at `+û·outR` vs `−û·outR`) is settled *at the composition layer*, not by changing the atom: the spec's `repel` mouth is precisely the term that reverses the re-entry, so no shipped force moves on any of the four planes and the golden stays byte-identical.
  - **Plumbing.** `PresetEntry` gains `pair` / `twist` / `scale` / `when`, and the `'@pair'` sentinel makes a preset inherit the host element's own `data-pair` — the blocker that kept #660 unshippable, since a preset table cannot know the partner selector. `expandPreset(name, host?)` takes the element's attrs to resolve it. No new `data-*` attribute: `data-pair` / `data-twist` / `data-scale` / `data-when` all already ship and are already documented.
  - **Ships inert.** With no `data-pair`, `@pair` resolves to nothing, `warp` no-ops, and the hot gate never opens — the element is a plain attract well. Every existing preset and force is byte-identical, and the ports carry no preset catalog, so this is a JS-plane change only.
...
- **Rust plane: the crates.io publishing path is decided and prepared — nothing published** (#1047). The crate is named `fundamental-core` (the crates.io spelling of `@fundamental-engine/core`; verified unclaimed by a read-only registry query, nothing reserved) and versioned in **lockstep** with `FIELD_VERSION`: `rust/Cargo.toml` carries the fleet version (`0.10.1`) via `[workspace.package]`, the crate gains a `FIELD_VERSION` constant and a `version_lockstep` drift guard mirroring the JS/Swift/Kotlin ones, and `RELEASING.md` step 4 now bumps four planes. `Cargo.toml` metadata (description, MIT, repository, homepage, docs.rs, keywords, categories, a crate-local README) makes `cargo package --list` and `cargo publish --dry-run` green; the monorepo-only golden test is excluded from the tarball. A new **dispatch-only, dry-run-by-default** `crates-io.yml` workflow is the publish path (a policy change, declared as such); it needs a maintainer-created `CARGO_REGISTRY_TOKEN` secret and is never triggered by a tag. Decision record: `docs/planning/rust-publishing-decision.md`. No npm package changes.
- **Maven Central publishing is prepared for the Kotlin artifacts** (#1092). `com.fundamental:fundamental-core` and `:fundamental-compose` now publish with everything the Sonatype Central Portal validates — a full POM (name, description, url, MIT license, developer, scm), `-sources` + `-javadoc` jars, checksums — and GPG signing from an **in-memory** key (`SIGNING_KEY` / `SIGNING_PASSWORD`; opt-in, unsigned when absent). New root Gradle tasks: `centralBundle` (stage + sign + zip into the Portal bundle, prints the manifest, **no network**) and `centralUpload` (the only task that talks to `central.sonatype.com`; refuses a `-SNAPSHOT` or an unsigned bundle), plus a **dispatch-only, dry-run-by-default** `maven-central.yml` workflow. GitHub Packages publishing on a release tag is unchanged — both targets share one publication. Going live still needs the maintainer's Portal account, namespace verification (the `com.fundamental` group requires owning fundamental.com — see `PUBLISHING.md`), and the secrets.

### Fixed

- **The Compose `FieldView` speaks the engine's render vocabulary** (#1158). The Compose host declared a *second*, unrelated `RenderMode` — `DOTS, TRAILS, LINKS, GLOW` — that did not import the engine's seven-member `com.fundamental.core.runtime.RenderMode`. Two same-named enums in one artifact set meant `RenderMode.DOTS` resolved to different types depending on the import, and `metaballs`, `voronoi` and `streamlines` were not merely undrawn but **inexpressible**: they were not members of the enum the `renderMode` parameter took, so no Compose app could ask for them. `FieldView` now takes the engine's enum and draws every mode — `METABALLS` (marching-squares iso-surface), `VORONOI` (nearest-site cell walls over the live swarm) and `STREAMLINES` (the probe-lattice force arrows, EMA-normalized), each using the same constants as the JS underlay and the Swift `CoreGraphicsRenderer` so the three planes agree visually — and honours `NONE` by drawing nothing while the simulation and every signal stay live. The geometry is ported into `:fundamental-core` (`engine/RenderModes.kt`: `marchingCell`, `splatDensity`, `nearestSite`/`nearestParticle`, `voronoiWalls`, `linkAlpha`), mirroring Swift's split of pure geometry from the drawing surface, and is unit-tested there. The host's `when` is now an exhaustive *expression* over the core enum, so a mode added to the engine fails the Compose build instead of silently drawing nothing.

  **Breaking for a Compose consumer, with a source-compatible bridge.** `com.fundamental.compose.RenderMode` is now a deprecated `typealias` to the engine's enum, so existing `RenderMode.DOTS` / `.TRAILS` / `.LINKS` keep compiling and name the same treatments. **`GLOW` is gone**: it existed on no other plane, had no entry in the cross-plane render vocabulary and no conformance coverage, so it could not become a member of the shared enum. **Migration:** `renderMode = RenderMode.DOTS, particleGlow = 1f` — the same soft additive bloom, expressed in the shared vocabulary. Whether `glow` should be *promoted* to the cross-plane vocabulary is a three-plane decision (engine + Swift + Kotlin, plus a conformance entry) and is left to a follow-up, not taken here. The host default stays `DOTS`, so a caller that never named a mode sees no change.

  The `render-host-modes` parity dimension — added by #1157 precisely to make this gap visible — now reads Kotlin 6 of 11 rather than 4, and the generator reads the modes the `Canvas` actually paints instead of a host enum that no longer exists.
...
- **A drawn diagnostic no longer perturbs the simulation** (#1155). The streamlines / flow / grid / path overlays, the field-line and force-vector readings, and `FieldHandle.sample()` all measure the field through `forceAt`, which ran every body's real `apply()` against the **caller's live `env`** and a shared probe particle whose `cap` was never cleared. So drawing the field *moved* it: a `sink` body's accretion budget advanced (`b.accreted += 1`) and could fire the live `supernova` — detonating a body that never absorbed a particle — while `diffuse`/`memory` wore the real scalar grids, a source could spawn, and every sample advanced the seeded `rng` a recorded run depends on. And because `cap` was never cleared, the first sink the probe met short-circuited `sink`/`warp` for the life of the process, so the drawn field went quietly wrong everywhere downstream of it. The probe now runs under its **own `Env`** — inert `spark`/`supernova`/`spawn`, scalar grids that read through but drop their deposits, and its own per-sample reseeded noise — and `cap` is cleared each sample, which is safe only because that path is now inert. `Env` gains an internal `probe` marker so a force that writes engine state outside the probe particle (only `sink`'s accretion today) skips that write on a reading; the diagnostics probe env behind `forceVectorAt`/`causalityAt` carries the same marker, where `sink` used to accrete and then trip over that bare env's missing `supernova` into a swallowing `catch`. **The drawn field's values are unchanged** for every non-mutating force, and the cross-plane conformance golden is byte-identical. The Swift port shares the live-env half of this bug (#1162).
- **Kotlin: drawing an overlay no longer detonates a real body on a later frame** (#1172, the Kotlin half of #1155). `forceAt` — the probe behind the streamlines / force-vector overlays (`Overlays`), the field-line tracer, `FieldController.sample()` and `FieldHandle.sample()` — ran every body's real `apply()` with a **fresh `Particle` per call**, so `sink`'s own `if (particle.cap != null …) return` guard never short-circuited and every probe point inside `absorbR` accreted into a **real body**, on every sample of every frame. Measured on a 12-frame streamlines walk over one capacity-4 sink: **420 accretions** where the correct count is zero — the same figure the Swift port measured (#1162), and against **1** in the JS engine, whose one shared probe particle carried `cap` for the life of the process and so over-accreted a body exactly once, self-limiting by accident. Kotlin's `forceAt` already built its own bare `Env()` rather than threading the caller's, so its `supernova` was a default no-op and no body detonated on the spot — which made this **harder** to diagnose, not milder: the corrupted `accreted` still drives the body's load metric and feedback vars and pushes it past `capacity`, so the **integrator's next genuine capture** detonates it. Measured on two identically seeded 30-frame runs with real matter, one drawing the overlay each frame and one not: **47 accretions and no supernova undrawn, 276 accretions and a detonation drawn**. The probe now runs under a deliberate inert env (`makeProbeEnv()` — no-op `spark`/`supernova`/`spawn`, a `NoopGrid`, and its own per-sample reseeded `rng` in place of the bare env's platform generator, so a drawn diagnostic is reproducible), plus an internal `Env.isProbe` marker that `sink` checks before accreting — because a direct body write goes through no service an inert env could stub. Mirrors the JS `Env.probe` (#1155) and Swift `Env.isProbe` (#1162) contract exactly: one writer, one reader. **The drawn vectors are unchanged** for every non-mutating force (`sink` contributes no velocity either way) and the cross-plane conformance golden is byte-identical.
