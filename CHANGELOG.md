...
- **Observer-driven measurement — the engine stops polling a page that is not moving** (#689). `getBoundingClientRect` forces the browser to flush layout, and the engine paid for one per body every 6th frame, forever, whether or not anything had changed. A host can now supply `observeBodies` (new optional `FieldHost` capability; `browserHost` backs it with `ResizeObserver` + `IntersectionObserver`) and the engine measures geometry when something was reported instead. **Measured: 10 rect reads per 60 frames → 2**, asserted as exact counts rather than described.
  - A measure was really two passes on one clock, and they belong on different ones. The **geometry** pass (position, extents, visibility) is the half that forces layout and now runs on demand. The **attribute** pass (`data-active` and the reactive `data-strength`/`range`/`spin`/`angle`) touches no layout and keeps the fixed cadence — `BodyHandle.set` and live `data-*` edits are documented as landing "within a frame on the measure cadence", and folding them into the slow pass would quietly have made that half a second.
  - **The poll is slowed, not removed, and that is deliberate.** Neither observer fires when an element merely *moves* — a sibling reflowing above it, a transform animation, a layout shift. A 30-frame safety cadence still catches what observation cannot see, so an unobservable move self-corrects in half a second instead of never. There is a test for exactly that case.
  - A body measured through a host-supplied `rect()` — a programmatic `addBody`, a Three.js mesh, a shadow-DOM body with a custom box — is neither observable nor cacheable: its provider can return something new on any frame and no browser observer will say so. One such body holds the whole geometry pass at the hot cadence, and such bodies are never handed to the observer (a real `ResizeObserver` would throw on `addBody`'s synthetic element).
  - **A host that does not observe is byte-identical to before**, not degraded — the gate is unconditionally true without the capability. `defineHost` deliberately does *not* fill a no-op default for this one member: an observer that never fires would read as "nothing ever changes", and the engine would slow its safety cadence on a signal that does not exist.
  - `hostCapabilities()` gains `bodyObservation`. `measureBodies` keeps its signature and behaviour for every existing caller; the two halves are exported as `measureBodyGeometry` / `refreshBodyAttrs`.
