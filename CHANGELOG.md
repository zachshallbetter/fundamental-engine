...
- **One render vocabulary: `RenderModeName` + `RENDER_MODE_LIST`, checked in both directions** (#1218). The render-mode half of the same problem #1215 fixed for overlay readings — and the worse half. Adding one mode meant editing the same list in **five** hand-maintained places, each failing silently and differently: rejected at construction, filtered out of the `<field-root>` attribute and quietly downgraded to `none`, or dropped from a Pattern's plan with `check:recipes` green.
  - The worst was the `<field-root>` getter: a declared return type and the runtime `v === '…'` chain that produces the value, **two independent lists on adjacent lines**, free to disagree while type-checking. Both are now the one list, so they cannot.
  - `satisfies` proves list ⊆ union; a type-level `Exclude<…> extends never` assertion proves union ⊆ list. Both directions were mutation-tested and both break the build.
  - `check:docs` and `gen:parity-matrix` both keep reading the **type declaration**, not the list: the list is what the code derives from, so pointing the gates at it would stop them being independent, and the exhaustiveness assertion already proves the two agree.
  - **No surface moved** — `data/parity-matrix.json` regenerates byte-identically, `check:recipes` produces no diff, and the Pattern routing of every mode is pinned as an exact table (including that `flow` has deliberately never been a Pattern layer).
