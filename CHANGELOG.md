...
- **A field that draws no matter can still draw a reading — `render: 'none'` no longer silences the overlay** (Field Surfaces lane S0, epic #732 / #297 §13.7). The engine's frame loop gated **both** surfaces on one condition (`ctx && cfg.render !== 'none' && …`), so a field in the signals-only mode — **the default since #538** — accepted `setOverlay('grid')`, resolved nothing, drew nothing, and warned about nothing. The two surfaces now gate separately: the underlay still needs a drawing render mode and a context; the overlay needs a resolved surface and an active reading. `render: 'none'` is a statement about **matter**, and the coupling to readings was one shared `if` that had hardened into a written guarantee.
  - **The canon is amended on the record, not quietly contradicted.** `forces-system.md` §13.7 used to promise that under `render: 'none'` *"`getContext('2d')` is never called — on the main canvas or the overlay canvas"* and that *"`setOverlay` is accepted but draws nothing while the mode is `'none'`"*. That clause is now **withdrawn in the document itself**, with the reasoning and a per-surface table of what each one still obeys; `invisible-fields.md` gains the matching note that "a field can render nothing and still be complete" is a claim about matter, not readings; `visualization-methods-taxonomy.md` records that placement is orthogonal to the underlay mode. The surviving half of the #297 guarantee is **pinned by test**: a signals-only field still acquires **no** context for its main canvas and leaves that backing store 0×0, reading or no reading.
  - **Nothing moves for a field that declares no reading**, which is every field that has not asked for one. `overlay` defaults to `'off'`; no canvas is requested from `overlayCanvasProvider` (the #676 pins in `option-seams.test.ts` are untouched), no context is acquired, nothing is drawn. The new cost on the default path is one null check per frame. The site's own `<field-root>` is `render="dots" overlay="off"` and is byte-identical.
  - **One documented non-identical case:** a caller who *injects* an `overlayBackend` into a `render: 'none'` field now receives `size(W, H, dpr)` calls it previously did not (the sizing path used to return early with no underlay context). No shipped host injects one.
  - **Swift moves in the same commit.** `FieldEngine`'s draw gate and `FieldField`'s managed-surface attach take the same shape, and `setOverlay` attaches a surface when a reading goes active — the Swift counterpart of core's lazy `ensureOverlaySurface`. New (additive) Swift surface: `OverlayInput.activeModes` / `.isActive`, now the one definition of "a reading is active" that both the host and the engine read. **Kotlin has no gate to relax**: its app hosts (`FieldView`) draw no readings at all today — only the JVM lab does, through its own enum — so this is honestly a two-plane change, and `overlay-modes` parity (9/9/9, vocabulary-only on Kotlin) is unaffected either way. The cross-plane conformance golden is untouched: no force, integrator or metric behaviour changed.

### Fixed

- **The lazily-resolved overlay surface obeys `dprCap` and the quality tier** (Field Surfaces lane S0, C-25). `ensureOverlaySurface()` sized the backing store from the **raw** `host.viewport().dpr`, while `sizeSurfaces()` clamped by `cfg.dprCap` (#410) and the tier's DPR ceiling (#413). A surface resolved off-cycle — the #676 lazy path, i.e. every `<field-root>` overlay — therefore ignored both until the next `resize()`: on a DPR-3 phone with the default cap of 2, a full-viewport `mix-blend-mode: screen` layer at 2.25× the pixels the underlay was allowed. There is now one `effectiveDpr()` definition and both paths resolve through it. `setDprCap` / `setQualityTier` also re-size a field whose only surface is the overlay, which the old `if (ctx)` guard could not reach.
