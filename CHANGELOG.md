...
- **Pointer dynamics — the cursor as a participant in the field** (#666). `field.pointer(x, y, opts?)` / `clearPointer()`. A `pointermove` handler is the whole integration, and the engine does two things with the positions: keeps a **real body** at the cursor carrying a force token you pick (`'repel'` by default, so a finger pushes matter aside; `'attract'` gathers, `'swirl'` stirs) — `query()` reports it like any other body, under the id `pointer` — and tracks its **velocity**, so matter near a moving cursor inherits the motion.
  - **The velocity is the whole point, and it is what `flowTo` cannot do.** A flow focus is a *place* the field bends toward: it behaves identically whether you slid it there or teleported it. A pointer that is not moving leaves **no wake at all**, a slow drift entrains gently, and a flick pulls a streak of matter behind it. That difference is what makes a cursor read as a finger through sand rather than a magnet being slid around, and it is asserted directly — two identical fields, one cursor dragged across and one parked at the destination.
  - Building the cursor through `addBody` is not a shortcut; it is what makes the claim true. The pointer goes through the same classification, measurement and reporting path as a `[data-body]`, so there is no second code path to keep in step with the first.
  - A pointer that stops being reported keeps its **position** but fades its **wake** out within ~0.25 s, so a cursor that left the window never drags matter forever — and that ageing runs even on a frozen field. Sampled speed is clamped, so a teleport (tab switch, remote desktop, a synthetic event) never becomes an impulse nothing can absorb.
- **Throw — `field.fling(el, vx, vy)`** (#666). Hand a `[data-move]` element the velocity it was travelling at when the user let go, in **px/second** (what a drag handler measures), and the field carries it on. It keeps its existing anchor spring and friction, so the throw arcs, decelerates and settles back into its layout slot — a release into the field's own dynamics, not an animation played over the top of them.
  - **Bounded by the mover contract, not by the velocity you hand it**: a `[data-move]` element never wanders past the element-offset integrator's 80 px ceiling, so this is a nudge with momentum, not a card sailing across the page. Past that ceiling more velocity buys *nothing*, which is pinned by a test because it is the first thing a caller will be surprised by.
  - Velocity is clamped at 3000 px/s, and the px/second → px/frame conversion lives in one named function: an unconverted value is 60× too large, which reads as "the throw is broken" rather than "the units are wrong".
  - All three methods are withheld from the read-only agent view — an agent reading the field never gets to move the user's cursor or throw their UI around.
