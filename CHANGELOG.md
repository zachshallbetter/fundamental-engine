...
- **Contained fields stop when their box scrolls out of view** (#672 lane S2, Field Surfaces program §6.1 **C-4**). A `containerHost` field used to run its frame loop forever — `hidden()` reported only `document.hidden` — so N cards on a page were N always-running rAF loops, each doing a full simulation, measurement and feedback pass every frame for a card nobody could see. The host now observes its own box and reports `hidden()` when it is off-screen, which is the contract the engine answers by cancelling its rAF.
  - **Measured end to end in a real browser**, counting the feedback sink's per-frame `style` writes on `/ai-evidence`: on `main` an off-screen contained field still wrote **13 times per 1.2 s**; with the gate it writes **0**, and resumes on return. That A/B is now an e2e test, not a claim.
  - **Deliberately stronger than `<field-root>`'s own IntersectionObserver**, which calls `setVisible(false)` — and `setVisible` gates the DRAW and nothing else (it sets `canvasVisible`, read only by the draw block; the simulation, measurement and feedback writes carry on). Draw-only is right for the page field: it is `position:fixed; inset:0`, so "not intersecting" means hidden or zero-sized rather than scrolled away, and its signals drive the whole page. A contained field has neither property.
  - A card resumes slightly **before** it scrolls into view (a 20% root margin), so the first frame a reader sees is a running field rather than a cold start. The observer fires the engine only on an edge, not on every threshold crossing, and a torn-down observer leaves the gate **open** rather than latched shut.
  - Opt out with `containerHost(el, { pauseOffscreen: false })`, or `runOffscreen: true` on the `bounds` door (`createField` / `new FieldField`), for a card whose field feeds a read-out elsewhere on the page or a `snapshot()` that must have no gap. A host with no `IntersectionObserver` keeps the old behaviour rather than a broken one.
