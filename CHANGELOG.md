...
- **Kotlin: `warp` stops being dead code — the wormhole pairing the port never had.** `warp` has been a registered Kotlin force token since the extended forces landed, and `WarpForce.apply` returned at its first guard — `if (!body.warpHas || particle.cap != null) return` — on **every frame of every Kotlin field**. `Body.warpHas` / `warpTarget` / `twist` / `warpScale` were declared, read by the force, and written by nothing: the comment above them promised state "resolved each scan (§22.3)" and that resolution step was never written. Kotlin had **no pairing concept at all** — no `pairBody`, no `pair` — so a consumer could not express a wormhole, a twist or an exit scale even in principle. The only writer of `warpHas` in the whole port was a hand-set test fixture, which is exactly why this shipped: the unit test passed while the runtime path never ran once.

  **The fix follows Swift, not JavaScript, and deliberately so.** JS resolves the pairing from a `data-pair` CSS selector against the DOM (`host.root.querySelector`) — meaningless on a platform with no DOM, which is why Swift has the CONSUMER assign `Body.pairBody` directly (`FieldLabKit/Scenes.swift`) and the engine merely read it each measure. Kotlin now does the same: `Body.pairBody` is the supplied reference, and `FieldController` resolves it into `warpTarget`/`warpHas` every tick, right after the measure loop settles the boxes.

  **New Kotlin surface, and that is all of it:** `Body.pairBody`, `BodyHandle.pairWith(other)` (null unpairs), and `BodySpec.twistDeg` / `BodySpec.warpScale`. `twistDeg` is degrees in and radians on the body — the JS scanner's `data-twist` conversion, and the same convention as the existing `angleDeg`. The pairing is **directed**, as on both other planes, so a two-way wormhole is two calls. Three differences from the Swift read are deliberate: the resolution is a **second pass** rather than part of the measure loop (Swift resolves inline, so a partner later in the body list is read at *last* frame's centre — order-dependent; JS runs `updateWarpTargets` after `measureBodies`, and Kotlin now matches JS here), the partner's **visibility gates the link** (JS's `if (b.pairBody.vis)`; an off-screen body exerts no force anywhere else in the engine and must not be a live teleport destination), and **severance happens at removal** — `removeBody` clears every inbound pairing, which is both the Kotlin form of JS's `isConnected` check and what releases the reference that Swift's `weak var` releases for free.

  **Pinned end to end, not by fixture.** The new `WarpPairingTests` builds a real field through the public API, registers two throats, pairs them, ticks, and asserts where matter actually lands — no test in it writes `warpHas`, `warpTarget` or `pairBody`. With the resolution pass removed, 5 of its 7 tests fail; the two that survive are the ones pinning that an *unpaired* throat stays inert. **No force, integrator or JS/Swift behaviour changed**, and a field with no pairing performs one boolean write per body per tick and is otherwise byte-identical — the cross-plane conformance golden does not move. `data/parity-matrix.json` claimed Kotlin supported `warp` while it was unreachable; that claim is **now true**, and the regenerated matrix records the supply surface (`body-spec` Kotlin 8 → 10, `body-handle` 6 → 7). Kotlin's programmatic `BodySpec` now reaches `twist`/`scale`, which JS's cannot — JS pairs only through the DOM.
