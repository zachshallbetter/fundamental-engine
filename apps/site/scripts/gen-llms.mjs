...
const DOC_DESC = {
  ...
  '/docs/concepts': 'the mental model in one narrative — bodies, the shared field context, reciprocity, the vocabulary lanes, the four-field translation system, and one ordinary element revealed as a field participant layer by layer',
  ...
  '/docs/api/declarative': 'the whole markup-authored surface in one place — the [data-body] contract, every force with its law and example, the presets, the feedback channels and the --field-* metric lane, conditions, formations, render/overlay vocab, palettes and the two custom elements, each with a generated JS/Swift/Kotlin support row',
  '/docs/api/imperative': 'the whole code-reached surface in one place — createField and every option, the whole FieldHandle, the sampling and observable reads, the agent view, policy and budgets, the body handle, the event bus, and the core type contracts, each with a generated JS/Swift/Kotlin support row',
  '/docs/api/utilities': 'seven web-only platform helpers — withFlip, allocateAttention, threadOverlay, bindFieldNav, QualityGovernor, textBodies, headlessHost. Shipped, none frozen',
  '/docs/api/stability': 'the frozen 0.x surface and compatibility rules — rendered from the same data CI enforces',
  '/docs/api/parity': 'which capabilities each plane actually has, generated from the port sources',
  ...
  '/docs/cookbook': 'the task-oriented depth layer — every pattern with a runnable example',
  '/docs/cookbook/signals-first': "the full simulation with nothing drawn: render:'none', headlessHost, and custom feedback sinks",
  '/docs/cookbook/contained-fields': 'a field scoped to one element (containerHost/bounds), shadow-DOM scope and portals, and the 130 x density rule',
  '/docs/cookbook/data-driven': 'bindData at scale: records become bodies, metrics become --field-* state, updates diff by id',
  '/docs/cookbook/reading-the-field': 'the read side worked through: probes, query, snapshot/diff, scalar grids, field channels, seeding, and agents without Three.js',
  '/docs/cookbook/conditions-and-formations': 'conditional gates (data-when, incl. per-particle selective gates) and formations (data-formation / setFormation)',
  '/docs/cookbook/workbench': 'the two-tier surface model: pick-one substrate, additive overlay stack, and the overlayCanvas requirement',
  '/docs/cookbook/performance-tuning': 'density, dprCap and qualityTier measured on a live pool, with inspectBudget',
  '/docs/cookbook/interface-chrome': 'bindFieldNav for navigation chrome, and the rules that keep a field accessible',
  '/docs/cookbook/framework-interop': 'SSR, hydration and islands: the attribute contract serializes, the runtime attaches after',
