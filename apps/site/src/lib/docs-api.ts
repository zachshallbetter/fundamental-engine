...
export const OPTIONS: OptionRow[] = [
  ...
  { name: 'mass', type: 'boolean', def: 'false', desc: 'First-class mass: particle mass ∝ size and body forces accelerate by a = F/m; dynamic bodies also get inertial mass ∝ rendered area (#872).' },
  { name: 'reaction', type: 'boolean', def: 'false', desc: 'Newtonian own-emission reaction (#873): a dynamic body feels the equal-and-opposite of the net impulse it imparts to nearby matter (a directional emitter recoils like a rocket). Best paired with mass. Experimental.' },
  ...
  { name: 'dprCap', type: 'number', def: '2', desc: 'Backing-store device-pixel-ratio ceiling (#410) — the dominant fill-rate lever. Effective DPR = min(devicePixelRatio, dprCap); capping at ~1.5 buys ~1.8× headroom on retina for a small perceptual softening. Runtime-settable via setDprCap. <field-root dpr-cap>.' },
...
export const HANDLE: MethodRow[] = [
  ...
  { sig: 'setRender(mode)', desc: 'Switch the underlay render mode (behind content): dots / trails / links / metaballs / voronoi / streamlines / flow / knockout / redshift / blackbody / depth.' },
  ...
  { sig: 'readParticleChannels(channels, out)', desc: "Read multiple named channels from live particles into caller-owned Float32Array buffers (column-wise: all particles' first channel, then second, etc.). Returns the particle count written. Channels: 'x' | 'y' | 'z' | 'vx' | 'vy' | 'heat' | 'size' | 'm' | 'id' | 'age' | 'charge'. Unknown channels write 0. Mirrors readParticles() agent-exclusion behavior. Zero-alloc alternative to readParticles when you need a subset of channels. Shipped-but-unfrozen." },
  { sig: 'registerOverlay(name, drawFn)', desc: 'Register a named custom overlay function — extends setOverlay beyond the built-in reading stack. Called each frame when name is in the active overlay stack (via setOverlay). drawFn receives the active RenderBackend, current Env, and canvas W/H. Returns an unregister function. Lets third-party packages (e.g. @fundamental-engine/three) publish custom overlay modes the host switches in by name. Shipped-but-unfrozen.' },
...
export const RENDER_MODES: { name: string; desc: string }[] = [
  ...
  { name: 'knockout', desc: 'Figure-ground inversion — the field paints a solid accent wash and matter is punched out as negative space (a print knockout). Clip the canvas to real type with a CSS mask for the field-inside-letters treatment; matter never assembles into letterforms.' },
  { name: 'redshift', desc: 'Dots tinted by spectral shift instead of the heat ramp — Doppler from each particle\'s radial velocity (receding reds, approaching blues) plus a gravitational red near body wells. The relativistic accretion-disk look.' },
  { name: 'blackbody', desc: 'Dots tinted by energy on a thermal ramp — near-black ember → deep red → orange → warm white → blue-white — with brightness rising with temperature. Physically-warm color, not radiometry.' },
  { name: 'depth', desc: 'The z lane made visible (2.5D) — far-to-near painter\'s sorting so near matter occludes far, perspective parallax toward the viewport centre, and defocus with distance. Pairs with a depth > 0 field; identical geometry to dots when flat.' },
];
...
export const FIELD_ROOT_ATTRS: { name: string; option: string; desc: string }[] = [
  ...
  { name: 'render', option: 'render', desc: 'Underlay render mode (none / dots / trails / links / metaballs / voronoi / streamlines / flow / knockout / redshift / blackbody / depth). Mirrors the render createField option.' },
  ...
  { name: 'ambient-orbit', option: 'ambientOrbit', desc: 'DECLARED resting-formation swirl on attract (#978); 0 for a purely radial attract. Mirrors the ambientOrbit createField option.' },
  { name: 'ambient-wander', option: 'ambientWander', desc: 'DECLARED resting-formation drift (#978); lower for a calmer rest, 0 to still it. Mirrors the ambientWander createField option.' },
...
// ── the imperative / observable vocabulary (docs-refactor Phase 3, #998) ────────────────────────
// The tables behind /docs/api/imperative. Like the render/overlay tables above, each of these is
// enumerated by `check:docs` against its source of truth, so a lane added to the engine cannot ship
// undocumented — and a name that drifts (as the event ids below did) fails CI instead of the reader.

/** Discrete events on `FieldHandle.on(type, cb)`. Truth: `FieldEventMap` in core/engine/events.ts. */
export const EVENTS: { name: string; payload: string; desc: string }[] = [
  { name: 'captured', payload: '{ body, count }', desc: 'A sink body captured matter — the rising edge of accretion. count is the matter held at that edge. Coalesced per (body, type) per frame, so a same-frame fill+release delivers once.' },
  { name: 'released', payload: '{ body, count }', desc: 'A sink body released what it held — the falling edge, or a supernova. count is what was held at the rising edge (or the number of particles ejected).' },
  { name: 'enter', payload: '{ body, other }', desc: "Another body crossed INTO this body's range (#441) — the “entered radius” trigger. Directed: the pair is keyed (body, other), so two counterparties in one frame stay distinct." },
  { name: 'exit', payload: '{ body, other }', desc: "Another body crossed OUT of this body's range (#441). Thresholded and debounced, so hover jitter does not spam the bus." },
  { name: 'met', payload: '{ a, b }', desc: 'Two bodies came into contact — their boxes touched or overlapped (#441), delivered once on the rising edge. Keyed on the unordered pair.' },
  { name: 'focus', payload: 'FocusEvent', desc: 'EXPERIMENTAL (focus/attention substrate). A focus() deposit landed — the write-back channel and its append-only provenance receipt. The payload is flat (no Body reference) so it serializes and mirrors 1:1 to the ports. Keyed by (target, source), so an operator and an agent deposit on the same body in one frame both survive.' },
];

/** `AgentCapability` grants for `forAgent`. Truth: the union in core/engine/types.ts. An ALLOW-LIST:
 *  a dimension not granted is stripped from every reading; granting can only tighten, never widen.
 *
 *  `status` is checked against `forAgent`'s implementation in core/engine/field.ts, not assumed —
 *  WIRED means the engine reads the grant, DECLARED means the token is part of the contract and
 *  nothing consults it yet. All nine are WIRED today — #1160 wired the last two (read:snapshots and
 *  read:diagnostics); the page states each status rather than letting the reader assume an allow-list
 *  entry is a control. */
export const AGENT_CAPABILITIES: { name: string; status: string; desc: string }[] = [
  { name: 'read:metrics', status: 'WIRED', desc: "Per-body and field metrics (density, load, temperature, salience, …). Without it every reading's metrics lane is stripped." },
  { name: 'read:relationships', status: 'WIRED', desc: 'The relationship graph — the edges between bodies, by id, with their strength and memory. Without it the lane is emptied and snapshots are forced to includeRelationships: false.' },
  { name: 'read:influences', status: 'WIRED', desc: 'Per-force attribution at a point: which force contributed what, by channel (linear Δv or thermal heat), from the impulse accumulator.' },
  { name: 'read:body-data', status: 'WIRED', desc: "A body's opaque data record. Withheld even when a profile or includeData asked for it unless this is granted AND the privacy budget allows it." },
  { name: 'read:projections', status: 'WIRED', desc: 'The registered projections (substrate 05) reported by query() and snapshot() — metadata only, never a write seam.' },
  { name: 'read:replay', status: 'WIRED (shape gate)', desc: 'The replay() method itself. One of the two members the facade omits entirely rather than redacting: without this grant, view.replay is undefined.' },
  { name: 'read:focus', status: 'WIRED (shape gate) · JS only', desc: 'EXPERIMENTAL. The focusState() digest and the per-source provenance split (WHO is focused) — the other shape gate: without it, view.focusState is undefined. The aggregate per-body salience still rides query()/snapshot() under the base grant. The Swift and Kotlin AgentCapability enums carry the other eight; this one is not ported.' },
  { name: 'read:snapshots', status: 'WIRED (shape gate)', desc: 'The snapshot() method itself — the portable capture. Without this grant view.snapshot is undefined: the surface is CLOSED, not emptied, because an empty capture is indistinguishable from an empty field. query() is unaffected, so a denied agent is barred from the capture, never blinded. Mirrored on Swift and Kotlin, where the member returns nil/null instead — a protocol or class member cannot vanish.' },
  { name: 'read:diagnostics', status: 'WIRED', desc: 'The raw particle pool inside a capture (includeParticles) — engine internal state rather than a modelled reading of bodies, relationships or metrics. Without it the pool is withheld even when an explicit flag or the debug profile asked for it (tightens, never widens). The port snapshots carry no particle lane yet, so on Swift and Kotlin the gate is wired at the option boundary and becomes observable when that lane lands.' },
];

/** `SnapshotProfile` presets for `snapshot({ profile })`. Truth: the union in core/engine/types.ts.
 *  A profile composes with the explicit include* flags and the privacy policy, always resolving to
 *  the TIGHTEST result — it can never widen past what policy or an explicit deny allows. */
export const SNAPSHOT_PROFILES: { name: string; desc: string }[] = [
  { name: 'debug', desc: 'Everything the call can legally include — body data still gated by the privacy budget. For a local inspector, not for export.' },
  { name: 'agent', desc: 'Ids, metrics, relationships, influences and projections; NO opaque body data. The shape meant to enter an agent turn.' },
  { name: 'bug-report', desc: 'Structural only — versions, formations, relationships, influences. No user data, so it can be attached to an issue.' },
  { name: 'public', desc: 'Minimal: ids and shape. No relationships, influences, metrics detail or data — safe to publish.' },
];

/** `FieldBudgets` lanes on `FieldPolicy.budgets`. Truth: the interface in core/engine/types.ts.
 *  Each is 0..1. WIRED = the engine reads it today; DECLARED = the lane is part of the contract and
 *  reserved, but nothing enforces it yet. Documented as declared rather than implied. */
export const BUDGETS: { name: string; status: string; desc: string }[] = [
  { name: 'motion', status: 'WIRED', desc: 'Cap on how much motion the field may express; 0 behaves as reduced-motion (frozen). Folds via min with the host reduced-motion state and perf pressure into the effective motion the integrator and easing read — reduced-motion always wins, so a policy can lower motion but never raise it.' },
  { name: 'privacy', status: 'WIRED', desc: 'Below PRIVACY_DATA_THRESHOLD (0.5) snapshots withhold body data — regardless of includeData, the profile, or a read:body-data grant. The tightest-wins rule in one number.' },
  { name: 'force', status: 'DECLARED', desc: 'Cap on applied force magnitude — the share of the impulse budget matter may absorb.' },
  { name: 'attention', status: 'DECLARED', desc: 'Cap on conserved-attention spend — the finite focus budget allocateAttention divides.' },
  { name: 'thermal', status: 'DECLARED', desc: 'Cap on thermal/heat accumulation the field may carry.' },
  { name: 'render', status: 'DECLARED', desc: 'Cap on render cost the field may spend (draw layers / fill). The policy-lane sibling of dprCap and qualityTier, which ARE wired.' },
  { name: 'accessibility', status: 'DECLARED', desc: 'Accessibility floor — the minimum non-motion legibility the field must preserve.' },
  { name: 'agentRead', status: 'DECLARED (gate wired at 0)', desc: 'Cap on how much field state agent readers may consume. The fractional gradient is a declared seam; the zero case IS enforced — budgets.agentRead === 0 closes forAgent to the most-restricted view.' },
];

/** `BodyHandle` — what `addBody(spec)` hands back. Truth: the interface in core/engine/types.ts. */
export const BODY_HANDLE: { name: string; desc: string }[] = [
  { name: 'data', desc: "The carried record (BodySpec.data), opaque to the engine. Mutable: assign to it and the next query()/snapshot() that includes data sees the new value." },
  { name: 'channels', desc: "This body's latest feedback channels, updated IN PLACE each frame — the same seven readings the DOM sink writes to CSS variables, handed to a non-DOM host directly. Read it; do not hold a copy." },
  { name: 'set', desc: 'Mutate force params live — { strength?, range?, angle? (degrees), spin?, color? } — applied within a frame on the measure cadence with no rescan(). Only the keys you pass change. A STRUCTURAL change (different tokens) still needs remove + addBody: tokens are classified once, not reactive.' },
  { name: 'remove', desc: 'Remove the body from the field. The handle is inert afterwards.' },
];
