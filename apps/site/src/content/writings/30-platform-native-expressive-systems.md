---
title: "Platform-Native Expressive Systems Without Replacing HTML"
description: "A portable interface principle: layer expressive behavior on semantic HTML while keeping the DOM canonical, bindings inspectable, and every expressive channel optional."
summary: "Paper 30 of the Fundamental expansion family generalizes the first eight papers' core claim into a renderer-agnostic, falsifiable model for treating relational meaning as a first-class interface substrate over semantic HTML."
date: 2026-06-27
category: research
...
# Platform-Native Expressive Systems Without Replacing HTML

> **Status: citation-complete expansion draft (editorial review copy).** Paper 30 of the Fundamental expansion family. This paper generalizes claims from the first eight Fundamental papers into a broader web-platform paper. It is not yet externally peer reviewed, and any empirical section below is a study design rather than a result.

**Author:** Zach Shallbetter  
**Series:** Fundamental Research Expansion Papers, Paper 30 of 30  
**Depends on:** Paper 01, Paper 05, Paper 16, Paper 21  
...
Advanced expressive behavior can be layered on semantic HTML without replacing the document with a canvas app or bespoke renderer, if the semantic source remains canonical and expressive surfaces are bound, inspectable, and optional. This paper isolates that claim from the implementation-specific language of the first Fundamental paper family and states it as a portable interface principle. The contribution is threefold. First, it defines the core concept in operational terms: what state must exist, where that state lives, who can read it, and what makes the claim falsifiable. Second, it positions the concept against adjacent interface traditions — web platform, semantic HTML, expressive systems, canvas, and renderer agnostic — so the difference is not merely terminological. Third, it sketches a concrete evaluation and conformance plan that can be run without assuming that Fundamental is the only possible implementation. The paper is therefore a bridge: grounded in Fundamental's shipped vocabulary and documentation, but written so the model can be argued, criticized, and reproduced outside that codebase.
...
The first Fundamental paper family approaches this problem through a relational field runtime. The flagship paper names the general model; the supporting papers test that model against reading, evidence, motion/accessibility, host portability, patterns, data binding, and diagnostics. This paper pulls one reusable claim out of that family: advanced expressive behavior can be layered on semantic html without replacing the document with a canvas app or bespoke renderer, if the semantic source remains canonical and expressive surfaces are bound, inspectable, and optional.
...
**Verified source role.** This paper's load-bearing background claims are keyed to: [webcomponents]; [csshoudini]; [scrollanim]; [prefers-reduced-motion]; [fundamental-site]; [cockburn2005].

**Remaining research gap.** The remaining research gap is implementation evidence across multiple production renderers: the principle is strongest when a semantic DOM substrate and alternative renderers are exercised by the same audited core.

## 2. Background and related work

This paper is a web-platform and architecture argument. Web Components, CSS Houdini, scroll-driven animation, and the reduced-motion media query support the claim that the web platform already provides semantic, styling, animation, and preference primitives that expressive systems should compose with rather than bypass [webcomponents; csshoudini; scrollanim; prefers-reduced-motion]. Fundamental's public site supplies the concrete system case: the DOM is the first surface, not the only surface, and renderer-agnostic behavior can still keep semantic HTML authoritative [fundamental-site]. Hexagonal architecture provides the broader architectural analogy for keeping the domain core separated from platform adapters [cockburn2005].
...
- Define platform-native expression as enhancement over semantic substrate.
- Position against canvas-only and framework-only expressive systems.
- Explain host-driven, DOM-canonical, multi-surface architecture.
- List constraints for accessibility, portability, and diagnostics.
...
Advanced expressive behavior can be layered on semantic HTML without replacing the document with a canvas app or bespoke renderer, if the semantic source remains canonical and expressive surfaces are bound, inspectable, and optional. The claim is broader than Fundamental but made concrete by it. The first paper family shows the pieces: reading memory, evidence graphs, reduced-motion equivalence, host-driven runtime structure, patterns, data binding, and diagnostics. This expansion paper turns those pieces into a general research question: what would interface systems look like if relational meaning were treated as a substrate rather than an ornament?
