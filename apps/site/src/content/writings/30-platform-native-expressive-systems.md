...
The first Fundamental paper family approaches this problem through a relational field runtime. The flagship paper names the general model; the supporting papers test that model against reading, evidence, motion/accessibility, host portability, recipes, data binding, and diagnostics. This paper pulls one reusable claim out of that family: advanced expressive behavior can be layered on semantic html without replacing the document with a canvas app or bespoke renderer, if the semantic source remains canonical and expressive surfaces are bound, inspectable, and optional.
