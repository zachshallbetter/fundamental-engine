---
title: "Field-Based Evaluation Methods for Interactive Systems"
description: "A portable methods framework for evaluating field-based interfaces — orientation, trust calibration, accessibility equivalence, time-to-diagnose, and semantic preservation, not just task completion."
summary: "Paper 26 generalizes the Fundamental family's measurement approach into a portable, falsifiable framework for evaluating field-based interfaces beyond task completion and preference."
date: 2026-06-27
category: research
...
# Field-Based Evaluation Methods for Interactive Systems

> **Status: citation-complete expansion draft (editorial review copy).** Paper 26 of the Fundamental expansion family. This paper generalizes claims from the first eight Fundamental papers into a broader methods paper. It is not yet externally peer reviewed, and any empirical section below is a study design rather than a result.

**Author:** Zach Shallbetter  
**Series:** Fundamental Research Expansion Papers, Paper 26 of 30  
**Depends on:** Paper 02, Paper 03, Paper 04, Paper 08  
...
Field-based interfaces require evaluation methods that measure orientation, memory, calibration, trust, accessibility equivalence, time-to-diagnose, semantic preservation, and cognitive load rather than only task completion and preference. This paper isolates that claim from the implementation-specific language of the first Fundamental paper family and states it as a portable interface principle. The contribution is threefold. First, it defines the core concept in operational terms: what state must exist, where that state lives, who can read it, and what makes the claim falsifiable. Second, it positions the concept against adjacent interface traditions — evaluation, methods, HCI studies, trust calibration, and non-inferiority — so the difference is not merely terminological. Third, it sketches a concrete evaluation and conformance plan that can be run without assuming that Fundamental is the only possible implementation. The paper is therefore a bridge: grounded in Fundamental's shipped vocabulary and documentation, but written so the model can be argued, criticized, and reproduced outside that codebase.
...
The first Fundamental paper family approaches this problem through a relational field runtime. The flagship paper names the general model; the supporting papers test that model against reading, evidence, motion/accessibility, host portability, patterns, data binding, and diagnostics. This paper pulls one reusable claim out of that family: field-based interfaces require evaluation methods that measure orientation, memory, calibration, trust, accessibility equivalence, time-to-diagnose, semantic preservation, and cognitive load rather than only task completion and preference.
...
**Verified source role.** This paper's load-bearing background claims are keyed to: [hornbaek2006]; [hart1988]; [brooke1996]; [signal-detection]; [brier1950]; [baayen2008]; [nosek2018]; [piaggio2012].

**Remaining research gap.** The remaining research gap is not source coverage but protocol validation: each paper in the family should pre-register which construct it measures and which metric is primary.

## 2. Background and related work

This methods paper is grounded in measurement rather than system novelty. HCI usability-measurement literature supports the need to choose measures that match the construct rather than treating usability as a single number [hornbaek2006]. NASA-TLX and SUS provide workload and perceived-usability instruments when those constructs are relevant [hart1988; brooke1996]. Signal-detection theory and Brier scoring support calibration and discrimination outcomes for evidence/trust tasks [signal-detection; brier1950]. Mixed-effects modeling, preregistration, and non-inferiority guidance support the proposed analysis and reporting practices for repeated-measures and equivalence-oriented studies [baayen2008; nosek2018; piaggio2012].
...
- Assemble a measurement framework from the first eight papers.
- Define families of outcomes: orientation, recall, calibration, diagnosis, semantic preservation, workload.
- Map outcomes to instruments and statistical designs.
- Provide preregistration templates for field-interface studies.
