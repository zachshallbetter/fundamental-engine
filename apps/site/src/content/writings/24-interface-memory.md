---
title: "Interface Memory: Worn Paths, Dwell, and Returning Context"
description: "A portable HCI principle drawn from the Fundamental field runtime: interfaces should remember meaningful attention — dwell, revisitation, reading trails — as a legible, inspectable memory surface."
summary: "An expansion position paper arguing that interfaces should treat accumulated meaningful attention (dwell, revisitation, reading trails, review context) as a first-class, inspectable memory substrate rather than an ornament, with a falsifiable model and a four-study evaluation plan."
date: 2026-06-27
category: research
...
# Interface Memory: Worn Paths, Dwell, and Returning Context

> **Status: citation-complete expansion draft (editorial review copy).** Paper 24 of the Fundamental expansion family. This paper generalizes claims from the first eight Fundamental papers into a broader hci paper. It is not yet externally peer reviewed, and any empirical section below is a study design rather than a result.

**Author:** Zach Shallbetter  
**Series:** Fundamental Research Expansion Papers, Paper 24 of 30  
**Depends on:** Paper 02, Paper 10, Paper 18  
...
Interfaces should remember meaningful attention, not only clicks: dwell, revisitation, reading trails, review context, and task history can become a legible memory surface that helps users return to where meaning accumulated. This paper isolates that claim from the implementation-specific language of the first Fundamental paper family and states it as a portable interface principle. The contribution is threefold. First, it defines the core concept in operational terms: what state must exist, where that state lives, who can read it, and what makes the claim falsifiable. Second, it positions the concept against adjacent interface traditions — memory, dwell, returning context, worn paths, and attention history — so the difference is not merely terminological. Third, it sketches a concrete evaluation and conformance plan that can be run without assuming that Fundamental is the only possible implementation. The paper is therefore a bridge: grounded in Fundamental's shipped vocabulary and documentation, but written so the model can be argued, criticized, and reproduced outside that codebase.
...
The first Fundamental paper family approaches this problem through a relational field runtime. The flagship paper names the general model; the supporting papers test that model against reading, evidence, motion/accessibility, host portability, patterns, data binding, and diagnostics. This paper pulls one reusable claim out of that family: interfaces should remember meaningful attention, not only clicks: dwell, revisitation, reading trails, review context, and task history can become a legible memory surface that helps users return to where meaning accumulated.
...
**Verified source role.** This paper's load-bearing background claims are keyed to: [grasse1959]; [dorigo2006]; [mangen2013]; [ackerman2011]; [pirolli1999].

**Remaining research gap.** The remaining research gap is to distinguish beneficial interface memory from distracting personalization: dwell traces must improve return/context tasks without creating clutter or false confidence.

## 2. Background and related work

This paper uses stigmergy and reading/navigation work to frame interface memory. Stigmergy and ant-colony optimization support the general mechanism of agents leaving traces that later guide behavior [grasse1959; dorigo2006]. Screen-reading and metacognitive-regulation studies support the motivation that digital reading can provide weaker external cues for place and prior engagement than paper in some contexts [mangen2013; ackerman2011]. Information-foraging theory supports the idea that trails, scent, and previously traversed paths can be made visible as navigation aids [pirolli1999].
...
- Define interface memory as accumulated interaction state with semantic decay.
- Distinguish memory from logs, browser history, and analytics.
- Describe worn paths for documents, dashboards, review queues, and learning interfaces.
- Propose refinding and context-resumption measures.
