---
title: "From Layout to Topology: Interfaces as Graphs of Meaning"
description: "A theory paper generalizing Fundamental's relational-field model into a portable principle: layout arranges boxes, topology exposes meaning relationships, and complex interfaces need both."
summary: "Paper 28 abstracts the Fundamental paper family's core insight into a renderer-agnostic interface principle that meaning relationships should be first-class, inspectable interface state rather than visual side effects, and proposes a four-term model (participant, signal, binding, equivalence) with an evaluation plan."
date: 2026-06-27
category: research
...
# From Layout to Topology: Interfaces as Graphs of Meaning

> **Status: citation-complete expansion draft (editorial review copy).** Paper 28 of the Fundamental expansion family. This paper generalizes claims from the first eight Fundamental papers into a broader theory paper. It is not yet externally peer reviewed, and any empirical section below is a study design rather than a result.

**Author:** Zach Shallbetter  
**Series:** Fundamental Research Expansion Papers, Paper 28 of 30  
**Depends on:** Paper 02, Paper 03, Paper 07, Paper 23  
...
Layout arranges visible boxes; topology exposes meaning relationships. Complex interfaces need both, because many of the relationships users reason about are not encoded by position alone. This paper isolates that claim from the implementation-specific language of the first Fundamental paper family and states it as a portable interface principle. The contribution is threefold. First, it defines the core concept in operational terms: what state must exist, where that state lives, who can read it, and what makes the claim falsifiable. Second, it positions the concept against adjacent interface traditions — topology, graphs, layout, meaning relationships, and information visualization — so the difference is not merely terminological. Third, it sketches a concrete evaluation and conformance plan that can be run without assuming that Fundamental is the only possible implementation. The paper is therefore a bridge: grounded in Fundamental's shipped vocabulary and documentation, but written so the model can be argued, criticized, and reproduced outside that codebase.
...
The first Fundamental paper family approaches this problem through a relational field runtime. The flagship paper names the general model; the supporting papers test that model against reading, evidence, motion/accessibility, host portability, patterns, data binding, and diagnostics. This paper pulls one reusable claim out of that family: layout arranges visible boxes; topology exposes meaning relationships. complex interfaces need both, because many of the relationships users reason about are not encoded by position alone.
...
**Verified source role.** This paper's load-bearing background claims are keyed to: [conklin1987]; [furnas1986]; [bostock2011]; [fruchterman1991]; [shneiderman1996].

**Remaining research gap.** The remaining research gap is comparative evaluation: topology-aware interfaces should be tested against layout-only baselines in document, dashboard, and evidence-graph tasks.

## 2. Background and related work

This paper is grounded in hypertext, overview navigation, and graph visualization. Hypertext and fisheye-view work support the claim that structure, navigation, and local context are longstanding interface concerns beyond static layout [conklin1987; furnas1986]. D3 and force-directed-layout work support the representation of relationships as graph/topological structure rather than only as spatial boxes [bostock2011; fruchterman1991]. Shneiderman's task-by-data taxonomy supports the broader information-visualization principle that different data structures and tasks call for different interface representations [shneiderman1996].
...
- Distinguish spatial layout from semantic topology.
- Show domains where topology is hidden by flat layout: documents, AI answers, dashboards, plans.
- Define topology surfaces: edges, clusters, trails, contradiction zones, memory paths.
- Propose topology-aware interaction patterns.
