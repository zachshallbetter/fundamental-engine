...
The first Fundamental paper family approaches this problem through a relational field runtime. The flagship paper names the general model; the supporting papers test that model against reading, evidence, motion/accessibility, host portability, recipes, data binding, and diagnostics. This paper pulls one reusable claim out of that family: anything that emits, receives, stores, transforms, or explains interface state can be an interface participant; rendering is only one participant role, not the definition of participation.
