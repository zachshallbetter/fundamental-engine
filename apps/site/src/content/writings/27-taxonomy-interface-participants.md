---
title: "A Taxonomy of Interface Participants"
description: "A portable taxonomy generalizing Fundamental's interface-participant model: anything that emits, receives, stores, transforms, or explains interface state participates — rendering is just one role."
summary: "Generalizes the Fundamental paper family into a falsifiable taxonomy of interface participants — defining participant, signal, binding, and equivalence — and a conformance/evaluation plan for treating relational meaning as a first-class substrate rather than an ornament."
date: 2026-06-27
category: research
...
# A Taxonomy of Interface Participants

> **Status: citation-complete expansion draft (editorial review copy).** Paper 27 of the Fundamental expansion family. This paper generalizes claims from the first eight Fundamental papers into a broader taxonomy paper. It is not yet externally peer reviewed, and any empirical section below is a study design rather than a result.

**Author:** Zach Shallbetter  
**Series:** Fundamental Research Expansion Papers, Paper 27 of 30  
**Depends on:** Paper 01, Paper 07, Paper 23, Paper 28  
...
Anything that emits, receives, stores, transforms, or explains interface state can be an interface participant; rendering is only one participant role, not the definition of participation. This paper isolates that claim from the implementation-specific language of the first Fundamental paper family and states it as a portable interface principle. The contribution is threefold. First, it defines the core concept in operational terms: what state must exist, where that state lives, who can read it, and what makes the claim falsifiable. Second, it positions the concept against adjacent interface traditions — agents, participants, elements, data records, events, and layout regions — so the difference is not merely terminological. Third, it sketches a concrete evaluation and conformance plan that can be run without assuming that Fundamental is the only possible implementation. The paper is therefore a bridge: grounded in Fundamental's shipped vocabulary and documentation, but written so the model can be argued, criticized, and reproduced outside that codebase.
...
The first Fundamental paper family approaches this problem through a relational field runtime. The flagship paper names the general model; the supporting papers test that model against reading, evidence, motion/accessibility, host portability, patterns, data binding, and diagnostics. This paper pulls one reusable claim out of that family: anything that emits, receives, stores, transforms, or explains interface state can be an interface participant; rendering is only one participant role, not the definition of participation.
...
**Verified source role.** This paper's load-bearing background claims are keyed to: [fundamental-engine-interaction]; [webcomponents]; [bostock2011]; [gossman2005].

**Remaining research gap.** The remaining research gap is classification reliability: independent reviewers should be able to apply the participant taxonomy to real interfaces and agree on what participates versus what merely renders.

## 2. Background and related work

This paper is primarily a proposed taxonomy grounded in Fundamental's interaction model. The internal interaction documentation supplies the participant vocabulary that the paper makes explicit: elements, particles, relationships, events, users, layout regions, and data records [fundamental-engine-interaction]. Web Components and MVVM sources provide external comparison points for component structure and view-model binding [webcomponents; gossman2005]. D3 supports the data-participation comparison: data can bind to visual marks, but the proposed taxonomy asks whether records and relationships should participate in interface state more broadly [bostock2011].
...
- Define participant roles: emitter, receiver, store, transformer, observer, explainer.
- Classify particles, elements, relationships, events, users, layout, and data.
- Separate visual presence from semantic participation.
- Specify minimal contracts for participation.
