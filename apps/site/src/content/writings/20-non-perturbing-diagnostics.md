...
The first Fundamental paper family approaches this problem through a relational field runtime. The flagship paper names the general model; the supporting papers test that model against reading, evidence, motion/accessibility, host portability, recipes, data binding, and diagnostics. This paper pulls one reusable claim out of that family: a diagnostic interface should reveal the behavior it observes without becoming part of the behavior it measures; diagnostic layers must be read-only, separable, and auditable.
