...
The first Fundamental paper family approaches this problem through a relational field runtime. The flagship paper names the general model; the supporting papers test that model against reading, evidence, motion/accessibility, host portability, recipes, data binding, and diagnostics. This paper pulls one reusable claim out of that family: portable ui behavior runtimes should isolate environment contact behind explicit host interfaces, so the behavior core can run across browser, headless, worker, native, and alternative render targets without reaching for globals.
