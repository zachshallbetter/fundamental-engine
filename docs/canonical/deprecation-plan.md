...
**living aliases** kept so that consumers who migrated to `field`/`field-*` naming but still read old
var names or import the old package name do not break.
...
- `@fundamental-engine/compat-core` / `-elements` / `-react` / `-vanilla` packages — gone (removed at
  ...
  *package* is `@fundamental-engine/platform` (row #3).
...
- **`--mass` CSS var (row #1) — doc-only.** The engine *writes* the var; a `var(--mass)` read happens in
...
- **`<field-field>` (row #2) — doc-only for now.** A `console.warn` on `connectedCallback` is feasible and
...
- **`@fundamental-engine/platform` (row #3) — WARNS.** The entry module emits a one-time dev-only
...
[`migration-0.x-to-1.0.md`](../analysis/migration-0.x-to-1.0.md); the **maintainer** file-level removal inventory
