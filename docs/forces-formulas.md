...
| **Vortex**<br>`vortex` | **[A]** | `data-spin="1"`<br>`data-strength="1.0"`<br>`data-range="320"` | $$r = \text{range} \cdot (\text{on}\ ?\ 1.4 : 1)$$<br>$$S' = S \cdot (\text{on}\ ?\ 2 : 1)$$<br>$\text{if}\ d \ge r:\ \text{skip}$<br>$$f = (1 - d/r)^{1.4} \cdot S' \cdot 0.45$$<br>$$v_x += (dy/d) \cdot f \cdot s + (dx/d) \cdot f \cdot 0.6$$<br>$$v_y += (-dx/d) \cdot f \cdot s + (dy/d) \cdot f \cdot 0.6$$<br>$\text{if}\ \text{on}: \text{heat} = \max(\text{heat}, (1 - d/r) \cdot 0.6)$ | Tangential swirl. Inward bias factor $0.6$ binds the orbit into a whirlpool. Range $\times 1.4$, strength $\times 2$ on engagement. |
