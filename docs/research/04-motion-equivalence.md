...
Relational DOM Interfaces](01-field-translation-runtime.md), establishes the model, the six-registry
platform, and the truth-mode vocabulary this paper assumes; see also [Reading Field](02-reading-field.md)
(the empirical reading study), [Evidence Fields](03-evidence-fields.md) (trust and provenance). See
the [series index](README.md).
...
(`packages/core/src/recipes/schema.ts`) makes `accessibility` a non-optional field of `FieldPattern`,
...
- **Recipe-schema accessibility field:** `packages/core/src/recipes/schema.ts` (`AccessibilityRecipe`,
  the required `accessibility` field on `FieldPattern`, and the validator clause forbidding a
  motion-only pattern).
