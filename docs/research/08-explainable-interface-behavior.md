...
the fact by a human reading source code. We present **field diagnostics**: the part of field-ui that
makes interface behavior inspectable. field-ui computes interface behavior as a relational field
...
field-ui makes this gap worse before it makes it better. A relational field (Paper 1) is *more*
...
field-ui's answer is that the field is not only a *mechanism* but an *explanation surface*. Because
...
> **Interfaces should not only respond — they should be able to explain their response. field-ui's
...
field-ui adopts this at the level of interface *behavior* rather than data: the thing being made
...
are the closest practical relatives. They explain *what the engine did*. field-ui's Inspector explains
...
systems, explanation interfaces for recommenders) frames the user-facing form of the problem. field-ui
...
the production behavior. field-ui's distinguishing stance is that the *same* field state drives the
...
property that is otherwise invisible. field-ui's diagnostics form a small, closed family. Each is
...
field-ui separates structure from cause (Paper 1, §3.3): there is a real `apply()` to attribute the
...
**Question.** When a field-ui interface behaves unexpectedly, do the diagnostics (causality,
...
**Design.** A within-subjects task study. Authors are given a field-ui page exhibiting a seeded behavior
...
and they routinely fall out of sync. field-ui's diagnostics are not a second artifact. They read the
...
Interfaces should be able to explain their responses, and on field-ui they can, because behavior is
...
classification beneath them gets wrong. The diagnostic framework is field-ui's answer to its own
