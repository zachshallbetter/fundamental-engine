...
**Companion papers (domain validators):** [Reading Field](02-reading-field.md) (reading);
[Evidence Fields](03-evidence-fields.md) (AI trust); Motion Equivalence (accessibility);
Host-Driven Runtime (architecture); Portable Field Patterns (authoring); Data as Field Participants
(data binding); Explainable Interface Behavior (diagnostics). See the [series index](README.md).
...
`FieldPattern` declares an id, a human intent, an optional natural-field classification, its
...
[`references.md`](references.md) must be resolved and verified before submission.
