 ...
 * effective DPR is min(devicePixelRatio, dprCap); the default ceiling is 1.5; setDprCap re-sizes now.
 */
...
test('the default ceiling is 1.5 (a 3× display caps at 1.5×)', () => {
  const { canvas, w } = trackedCanvas();
  const field = createField(canvas, { host: host(3), render: 'dots' });
  try {
    assert.equal(w(), Math.floor(1000 * 1.5), `3× display → capped at 1.5× (${w()})`);
  } finally { field.destroy(); }
});

test('setQualityTier caps the effective DPR per tier and restores reversibly (#413)', () => {
  const { canvas, w } = trackedCanvas();
  const field = createField(canvas, { host: host(2), render: 'dots' }); // dprCap default 1.5 → 1500
  try {
    assert.equal(w(), Math.floor(1000 * 1.5), 'tier 0: configured DPR (1.5×)');
    field.setQualityTier(1); assert.equal(w(), Math.floor(1000 * 1.25), 'tier 1 → 1.25×');
    field.setQualityTier(2); assert.equal(w(), Math.floor(1000 * 1.1), 'tier 2 → 1.1×');
    field.setQualityTier(3); assert.equal(w(), 1000, 'tier 3 → 1×');
    field.setQualityTier(0); assert.equal(w(), Math.floor(1000 * 1.5), 'tier 0 restores full quality');
  } finally { field.destroy(); }
});
