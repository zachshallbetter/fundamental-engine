...
export interface ForcePassport {
  ...
  /** the dimensions this force COUPLES — cross-links so a change in one drives a change in another
   *  (the coupling passport, substrate doc 04 / dimensional-coupling). A speed-conserving force redirects
   *  velocity, coupling the `linear`-velocity components (direction); a torque-style force couples
   *  `linear`→`angular`. Required for any force the `field/no-dimension-coupling-without-passport` lint
   *  flags as a coupler (today: every `conservesSpeed` force). Omitted ⇒ the force couples nothing. */
  couplesDimensions?: readonly string[];
  designUse: string;
  physicsNote: string;
}
...
const ROWS: Row[] = [
  ...
  { token: 'wall', label: 'Wall', family: C, klass: 'A', truthMode: 'designed', ownsField: false, usesFieldAt: false, movesParticles: true, doesWork: false, conservesSpeed: true, couplesDimensions: ['linear'], requiresCharge: false, requiresVelocity: true, affectsNeutralMatter: true, designUse: 'bounce matter off a boundary', physicsNote: 'velocity-replacing elastic reflection (kinematic, speed preserved)' },
  ...
  { token: 'relief', label: 'Relief', family: E, klass: 'C', truthMode: 'physical', ownsField: false, usesFieldAt: false, movesParticles: true, doesWork: true, conservesSpeed: false, requiresCharge: false, requiresVelocity: false, affectsNeutralMatter: true, designUse: 'downhill flow, drainage, wind over relief — terrain the HOST owns, coupled as cause', physicsNote: 'transport down a declared scalar potential: Phi = G*h from an addField channel, rasterised into a held grid; v += -grad(Phi)*S. truthMode physical is legal ONLY because the entire structure is host-supplied — with no channel the force is a pure no-op and can decorate nothing (the wallpaper rule). The 1/m rescale an additive force gets makes heavy matter slide slower than a = -g*grad(h): an Idealization, not free fall.' },
  { token: 'warp', label: 'Warp', family: E, klass: 'A', truthMode: 'designed', ownsField: false, usesFieldAt: false, movesParticles: true, doesWork: false, conservesSpeed: true, couplesDimensions: ['linear'], requiresCharge: false, requiresVelocity: false, affectsNeutralMatter: true, designUse: 'relocate matter to a paired throat (a wormhole)', physicsNote: 'velocity-replacing teleport (kinematic): repositions to the data-pair body and rotates velocity by data-twist; speed preserved, count conserved' },
];
