...
The framework-free TypeScript wrapper for [forces-ui](../core) — a reciprocal DOM-physics
field as a typed `ForcesField` class and the imperative `mountField()`. Elements you mark
...
This is the same engine the `<forces-field>` custom element and the React `<ForcesField>`
...
`new ForcesField()` builds a fixed, full-viewport canvas behind your page and starts the
...
```ts
const field = new ForcesField({ canvas: myCanvas, density: 1.2 });
```
...
```ts
import { mountField } from '@forces-ui/vanilla';
...
```ts
import { createField } from '@forces-ui/vanilla';
