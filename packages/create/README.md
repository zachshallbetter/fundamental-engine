...
Three variants, all **signals-first** (the field draws nothing by default — it writes `--field-*` CSS variables your styles read; particles are one opt-in surface):
...
Each scaffolds a minimal Vite app you run with `npm install && npm run dev`. Every template is explicit about `render`, so it behaves the same whichever engine version it resolves to.
