...
export const EXPERIMENTAL = [
  ...
  { item: 'FieldHandle.scrollV()', status: 'partial', note: 'Shipped in @field-ui/core; returns the engine\'s EMA scroll velocity in px/frame (refresh-rate dependent — reads roughly half on a 120 Hz display). Mirrored as --field-scroll-v on :root by the platform runtime. Signature stable; semantics (the unit may normalize to px/ms, EMA factor) may refine before 1.0.' },
  { item: "render mode 'none' (signals-only engine)", status: 'partial', note: 'Shipped in @field-ui/core (#297): createField({ render: \'none\' }) runs the full simulation + feedback pipeline but never acquires a canvas context, never sizes the backing store (it stays 0×0), and never draws — the field exists purely as signals (--d, --load, --lit, capture events, scrollV). setRender FROM \'none\' acquires the context lazily; setRender TO \'none\' at runtime stops drawing but keeps an already-acquired context. Accepted by <field-root render="none">. Shipped-but-unfrozen; may refine before being added to the frozen list.' },
  { item: 'QualityGovernor / field:quality-tier', status: 'partial', note: 'Tier detection (0-3) ships in @field-ui/platform; the <field-root> runtime throttles its own tick cadence at tiers 2-3 and emits field:quality-tier. Engine-side degradation (render simplification, particle caps) is the embedder\'s to wire; unfrozen.' },
  { item: 'performance budget (inspectBudget / DEFAULT_BUDGET / createFieldPerf)', status: 'partial', note: 'inspectBudget(), withinBudget(), BudgetFinding, and DEFAULT_BUDGET ship in @field-ui/core. QualityGovernor covers adaptive tier detection. The FieldPerf frame-duration split now ships in @field-ui/platform as createFieldPerf() — pure timing math lifted from the site DataConsole prototype (rolling delta window of 180, nearest-rank-floor percentiles, budget = median of the first 30 clean deltas, dropped = delta > budget×1.5, gaps > 500 ms skipped as discontinuities; callers feed rAF timestamps — no rAF of its own). The LoAF / long-task split stays page-side (no PerformanceObserver in this slice). Unfrozen — option/snapshot shapes may refine before 1.0.' },
  ...
  { item: 'GPU / WebGPU backend', status: 'planned', note: 'a named direction (VisualBindingRegistry mentions WebGL); the six shipped drawing render modes are CPU/canvas (the seventh, \'none\', draws nothing).' },
